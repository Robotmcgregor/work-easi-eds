...
def find_closest_date_match(target_date: str, available_dates: List[str], max_days: int = 7) -> Tuple[Optional[str], int]:
...
def main():
    ...
    for tile_id, dates in fmask_by_tile.items():
        print(f"  Tile {tile_id}: {len(dates)} unique dates, {sum(len(files) for files in dates.values())} total files")
        ...
        try:
            ...
            if fc_tile in fmask_by_tile:
                ...
                if not fmask_match:
                    ...
                    closest_date, diff_days = find_closest_date_match(fc_date, available_dates, max_days=7)
            ...
            if fmask_match:
                ...
                try:
                    ...
                    result['dimension_match'] = result['fc_dims'] == result['fmask_dims']
                    
                    # Check coordinate proximity (within 1000m)
                    coord_diff = ((result['fc_coords'][0] - result['fmask_coords'][0]) ** 2 + 
                                 (result['fc_coords'][1] - result['fmask_coords'][1]) ** 2) ** 0.5
        ...
        if result['fmask_file'] and not result['dimension_match']:
            ...
            if dim_mismatch_count >= 5:  # Show only first 5
                if dimension_mismatches > 5:
                    print(f"... and {dimension_mismatches - 5} more dimension mismatches")
                break
    ...
    for result in match_results:
        if result['fmask_file'] and not result['coord_match'] and result['dimension_match']:
            fc_coords = result['fc_coords']
            fmask_coords = result['fmask_coords']
            coord_diff = ((fc_coords[0] - fmask_coords[0]) ** 2 + 
                         (fc_coords[1] - fmask_coords[1]) ** 2) ** 0.5
    ...
    success_count = 0
    for result in match_results:
        if (result['fmask_file'] and result['dimension_match'] and 
            result['coord_match'] and result['match_type'] == 'exact'):
