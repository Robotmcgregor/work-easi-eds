...
def generate_summary_report():
    ...
    with db_manager.get_session() as session:
        ...
        for run in runs:
            ...
            results = (
                session.query(NVMSResult).filter(NVMSResult.run_id == run.run_id).all()
            )
            ...
            if results:
                ...
                total_detections = sum(
                    (r.cleared or 0) + (r.not_cleared or 0) for r in results
                )
                ...
                print(
                    f"  Time period: {min(dates).strftime('%Y-%m-%d')} to {max(end_dates).strftime('%Y-%m-%d')}"
                )
        ...
        for date in sorted(by_date.keys(), reverse=True):
            ...
            for tile_id, info in sample_tiles:
                print(
                    f"  {tile_id} (P{info['path']}/R{info['row']}) - {info['last_run']} by {info['last_analyst']}"
                )
        ...
        if unprocessed_tiles:
            ...
            for tile_info in sample_unprocessed:
                tile_id = next(
                    tid for tid, info in tile_status.items() if info == tile_info
                )
        ...
        multi_run_tiles = [
            (tid, info) for tid, info in tile_status.items() if info["total_runs"] > 1
        ]
        ...
        for tile_id, info in multi_run_tiles[:10]:
            print(
                f"  {tile_id} (P{info['path']}/R{info['row']}): {info['total_runs']} runs, last: {info['last_processed'].strftime('%Y-%m-%d')} ({info['last_run']})"
            )

        # High detection tiles
        high_detection_tiles = (
            session.query(NVMSResult)
            .filter(NVMSResult.cleared > 20)
            .order_by(NVMSResult.cleared.desc())
            .all()
        )
        ...
        if high_detection_tiles:
            ...
            for result in high_detection_tiles[:10]:
                ...
                print(
                    f"  {result.tile_id}: {result.cleared} cleared / {total} total ({result.run_id}, {result.analyst})"
                )
        ...
        print(
            f"• Most recent processing: {max(last_dates).strftime('%Y-%m-%d') if processed_tiles else 'None'}"
        )
        ...
        if total_detections_all:
            ...
            all_total = sum(
                (r.cleared or 0) + (r.not_cleared or 0) for r in total_detections_all
            )
            print(f"• Total detections across all runs: {all_total:,}")
            print(
                f"• Total cleared detections: {all_cleared:,} ({all_cleared/all_total*100:.1f}%)"
            )
