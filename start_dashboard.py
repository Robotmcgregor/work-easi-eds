...
if __name__ == "__main__":
    ...
    app.layout = html.Div(
        [
            ...
            dcc.Interval(
                id="interval", interval=10000, n_intervals=0
            ),  # Update every 10s
        ]
    )
                    ...
                    for det in detections[:100]:  # Limit to first 100 for performance
                        if det.geom_geojson:
                            try:
                                ...
                                logger.error(
                                    f"Error processing detection geometry: {e}"
                                )
                                continue
                    ...
                    if det_data:
                        ...
                        fig.add_trace(
                            go.Scattermapbox(
                                ...
                                text=det_df["tile_id"],
                                hovertemplate="<b>Detection</b><br>Tile: %{text}<br>Run: "
                                + det_df["run_id"].astype(str)
                                + "<extra></extra>",
