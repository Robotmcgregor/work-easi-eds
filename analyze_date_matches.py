...
def main():
    ...
    for fc_path, fc_file, fc_date in fc_files:
        ...
        if fc_date in fmask_dates_set:
            ...
            closest_date, diff_days = find_closest_date(
                fc_date, fmask_dates, max_days=7
            )
            if closest_date:
                proximity_matches += 1
                proximity_match_details.append(
                    (
                        fc_file,
    ...
    if proximity_match_details:
        ...
        for i, (fc_file, fc_date, fmask_date, diff) in enumerate(
            proximity_match_details[:5]
        ):
    ...
    if fmask_dates:
        ...
        print(
            f"FMASK DATE RANGE: {fmask_start.strftime('%Y-%m-%d')} to {fmask_end.strftime('%Y-%m-%d')}"
        )
    ...
    for year in sorted(set(fc_by_year.keys()) | set(fmask_by_year.keys())):
        ...
        print(
            f"{year}   {fc_count:3d}   {fmask_count:3d}     {exact:2d}       {prox:2d}       {none:2d}"
        )
