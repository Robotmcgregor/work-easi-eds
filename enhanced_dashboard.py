...
if __name__ == "__main__":
    ...
    app.layout = html.Div([
        html.H1("EDS - Early Detection System", style={'textAlign': 'center', 'color': '#2c3e50', 'marginBottom': 20}),
        html.P("Land Clearing Detection Dashboard for Australia", style={'textAlign': 'center', 'marginBottom': 30}),
        ...
        dcc.Interval(id='interval', interval=30000, n_intervals=0)  # Update every 30s
    ], style={
    ...
    @app.callback(
        Output('run-breakdown-chart', 'figure'),
        [Input('interval', 'n_intervals')]
    )
    def update_run_breakdown(n_intervals):
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                if not run_data:
                    return go.Figure().add_annotation(text="No NVMS data", xref="paper", yref="paper", x=0.5, y=0.5)
            ...
            return go.Figure().add_annotation(text=f"Error: {str(e)}", xref="paper", yref="paper", x=0.5, y=0.5)
    ...
    def update_timeline(n_intervals):
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                if not results:
                    return go.Figure().add_annotation(text="No timeline data", xref="paper", yref="paper", x=0.5, y=0.5)
    ...
    def update_map(n_intervals):
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                if not tiles:
                    return go.Figure().add_annotation(text="No tiles found", xref="paper", yref="paper", x=0.5, y=0.5)
                    ...
                    for _, tile in tiles_with_bounds.iterrows():
                        try:
                            ...
                            fig.add_trace(go.Scattermapbox(
                                ...
                                lon=lons,
                                lat=lats,
                                line=dict(width=2, color=color_map[tile['last_run']]),
                                name=tile['last_run'],
                                showlegend=False,
                            ...
                            logger.error(f"Error adding boundary for tile {tile['tile_id']}: {e}")
                            continue
                else:
                    ...
                    for run_type in color_map.keys():
                        ...
                        if len(run_tiles) > 0:
                            fig.add_trace(go.Scattermapbox(
                                ...
                                lon=run_tiles['lon'],
                                mode='markers',
                                marker=dict(size=8, color=color_map[run_type], opacity=0.7),
