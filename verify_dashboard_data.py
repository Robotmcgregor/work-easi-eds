...
def main():
    ...
    with db.get_session() as session:
        ...
        active_tiles = session.query(LandsatTile).filter(LandsatTile.is_active == True).count()
        ...
        print(f"   Shows: {active_tiles} tile markers + {detections} detection overlays")
        print(f"   Basemap: OpenStreetMap (free)")
        
        # Status breakdown
        if active_tiles > 0:
            status_counts = {}
            for tile in session.query(LandsatTile).filter(LandsatTile.is_active == True).all():
                status = tile.status
