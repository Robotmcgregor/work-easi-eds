...
if __name__ == "__main__":
        ...
        [
            ...
            html.Div(
                [
                    ...
                    html.P(
                        "Early Detection System - Australia Landsat Tiles",
                        style={
                            "textAlign": "center",
                            "color": "#7f8c8d",
                            "marginBottom": 30,
                            ...
                            html.Div(
                                [
                                    html.Div(
                                        [
                                            html.H3(
                                                id="field-visit-count",
                                                children="...",
                                                style={"margin": 0, "color": "#9b59b6"},
                                            ),
                                            html.P(
                        ...
                        [
                            html.Div(
                                [
                                            ...
                                            html.Div(
                                                [
                                                    ...
                                                    dcc.RadioItems(
                                                        ...
                                                        options=[
                                                            {
                                                                "label": " Confirm Clearing",
                                                                "value": "confirmed",
                                                            },
                                                            {
                                                                "label": " Reject Detection",
                                                                "value": "rejected",
                                                            },
                                                            {
                                    ...
                                    # Status message
                                    html.Div(
                                        id="submit-status", style={"marginTop": 15}
                                    ),
                                ],
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                pending_qc = (
                    ...
                    .filter(QCValidation.qc_status == QCStatus.PENDING.value)
                    .count()
                )
                confirmed_qc = (
                    session.query(QCValidation)
                    .filter(QCValidation.qc_status == QCStatus.CONFIRMED.value)
                    .count()
                )
                rejected_qc = (
                    session.query(QCValidation)
                    .filter(QCValidation.qc_status == QCStatus.REJECTED.value)
                    .count()
                )
                field_visits = (
                    session.query(QCValidation)
                    .filter(QCValidation.requires_field_visit == True)
                    .count()
                )
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                if "tiles" in show_layers:
                    ...
                    print(
                        f"EDS: Loading {len(tiles)} tiles (size: {marker_size}px at zoom {zoom})"
                    )
                    ...
                    if tile_lats:
                        fig.add_trace(
                            go.Scattermapbox(
                                ...
                                mode="markers",
                                marker=dict(
                                    size=marker_size, color=tile_colors, opacity=0.8
                                ),
                                text=tile_texts,
                                name="EDS Tiles",
                ...
                fig.update_layout(
                    ...
                    height=700,
                    title=f'EDS in Action - {map_style.replace("-", " ").title()}',
                    showlegend=True,
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                detections = (
                    ...
                    .outerjoin(QCValidation)
                    .filter(QCValidation.id.is_(None))  # No QC record exists
                    .limit(50)
                    .all()
    ...
    @app.callback(
        Output("detection-details", "children"), [Input("detection-dropdown", "value")]
    )
    def update_detection_details(detection_id):
        if not detection_id or detection_id == "error":
            return html.Div(
                "Select a detection to see details.", style={"color": "#7f8c8d"}
            )
        ...
        try:
                    ...
                    [
                        ...
                        html.P(
                            [
                                ...
                                html.Strong("Run: "),
                                (
                                    detection.run_id.replace("NVMS_QLD_", "")
                                    if detection.run_id
                                    else "Unknown"
                                ),
                                html.Br(),
                                ...
                                (
                                    str(detection.properties)
                                    if detection.properties
                                    else "No additional data"
                                ),
                                html.Br(),
                                html.Strong("Imported: "),
                                (
                                    detection.imported_at.strftime("%Y-%m-%d %H:%M")
            ...
            return html.Div(
                f"Error loading detection details: {e}", style={"color": "#e74c3c"}
            )
    ...
    @app.callback(
        ...
        [Input("submit-review", "n_clicks"), Input("clear-form", "n_clicks")],
        [
            ...
            try:
                ...
                with db.get_session() as session:
                    ...
                    if not detection:
                        return (
                            html.Div(
                                "Detection not found.", style={"color": "#e74c3c"}
                            ),
                            reviewer_name,
                            validation_decision,
                            confidence,
                    ...
                    qc_validation = QCValidation(
                        ...
                        requires_field_visit=(
                            "field_visit" in field_visit if field_visit else False
                        ),
    ...
    @app.callback(
        ...
        [Input("update-interval", "n_intervals"), Input("submit-review", "n_clicks")],
    )
    def update_recent_qc_activity(n_intervals, submit_clicks):
        try:
            ...
            with db.get_session() as session:
                ...
                if not recent_qc:
                    return html.Div(
                        "No recent QC activity.", style={"color": "#7f8c8d"}
                    )
                        ...
                        activity_items.append(
                            html.Div(
                                [
                                    ...
                                    html.Br(),
                                    html.Small(
                                        f"By {qc.reviewed_by} on {qc.reviewed_at.strftime('%Y-%m-%d %H:%M')}"
                                    ),
                                ],
                                style={
                                    "padding": 8,
