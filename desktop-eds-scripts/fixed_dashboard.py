...
if __name__ == "__main__":
        ...
        [
            ...
            html.Div(
                [
                    ...
                    html.P(
                        "All Australia Landsat Tiles with Working Controls",
                        style={
                            "textAlign": "center",
                            "color": "#7f8c8d",
                            "marginBottom": 30,
            ...
            html.Div(
                [
                    html.Div([dcc.Graph(id="runs-chart")], className="six columns"),
                    html.Div([dcc.Graph(id="time-chart")], className="six columns"),
                ],
                className="row",
                    ...
                    html.Div(
                        [
                            ...
                            dcc.Checklist(
                                ...
                                options=[
                                    {
                                        "label": " All Tile Boundaries",
                                        "value": "all_tiles",
                                    },
                                    {
                                        "label": " EDS Processed Tiles Only",
                                        "value": "eds_tiles",
                                    },
                                    {
            ...
            # Map
            html.Div([dcc.Graph(id="fixed-map", style={"height": "700px"})]),
    ...
    @app.callback(
        Output("runs-chart", "figure"), [Input("update-interval", "n_intervals")]
    )
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                for i, run_num in enumerate([1, 2, 3]):
                    ...
                    tiles = (
                        session.query(NVMSResult)
                        .filter(NVMSResult.run_id == run_id)
                        .count()
                    )
                    detections = (
                        session.query(NVMSDetection)
                        .filter(NVMSDetection.run_id == run_id)
                        .count()
                    )

                    data.append(
                        {
                            "Run": f"EDS Run {run_num}",
                            "Tiles": tiles,
    ...
    @app.callback(
        Output("time-chart", "figure"), [Input("update-interval", "n_intervals")]
    )
    ...
    @app.callback(
        [Output("fixed-map", "figure"), Output("map-state", "data")],
        [
    ...
    def update_fixed_map(
        map_style, show_layers, zoom_qld_clicks, zoom_aus_clicks, n_intervals
    ):
        try:
            config = get_config()
            db = DatabaseManager(config.database.connection_url)
            ...
            center_lat, center_lon, zoom = (
                ...
                135,
                5,
            )  # Default Australia view (increased zoom)
            ...
            if ctx.triggered:
                ...
                if button_id == "zoom-qld":
                    center_lat, center_lon, zoom = (
                        ...
                        145,
                        7,
                    )  # Queensland (increased zoom)
                elif button_id == "zoom-aus":
                    center_lat, center_lon, zoom = (
                        -25,
                        135,
                        5,
                    )  # Australia (increased zoom)
            ...
            with db.get_session() as session:
                ...
                if "all_tiles" in show_layers:
                    tiles = session.query(
                        LandsatTile
                    ).all()  # Get ALL tiles - no limit!
                    print(f"Loading {len(tiles)} tiles for display")

                    # Color code: Black = no runs, Yellow = Run 1, Orange = Run 2, Red = Run 3
                    run_colors = {
                        "No runs": "rgba(64,64,64,0.8)",
                        "Run 1": "rgba(255,255,0,0.9)",
                        "Run 2": "rgba(255,165,0,0.9)",
                        "Run 3": "rgba(255,0,0,0.9)",
                    }
                        ...
                        if tile.center_lat and tile.center_lon:
                            ...
                            fig.add_trace(
                                go.Scattermapbox(
                                    ...
                                    name=run_type,
                                    showlegend=run_type
                                    not in [trace.name for trace in fig.data],
                ...
                if "eds_tiles" in show_layers:
                    eds_tiles = (
                        session.query(LandsatTile).join(NVMSResult).distinct().all()
                    )
                ...
                if map_style == "white-bg":
                    ...
                    fig.update_layout(
                        ...
                        height=700,
                        title=f"Australia Landsat Tiles - Satellite View",
                        showlegend=True,
                    ...
                    fig.update_layout(
                        ...
                        height=700,
                        title=f'Australia Landsat Tiles - {map_style.replace("-", " ").title()}',
                        showlegend=True,
                        margin=dict(r=0, t=40, l=0, b=0),
                        uirevision="constant",
                    )
            ...
            empty_fig.add_annotation(
                text=f"Map Error: {e}", xref="paper", yref="paper", x=0.5, y=0.5
            )
            return empty_fig, {}
