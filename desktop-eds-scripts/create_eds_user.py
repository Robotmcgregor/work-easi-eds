...
def create_eds_user():
    ...
    try:
        ...
        if user_exists:
            print("WARNING: User 'eds_user' already exists. Updating permissions...")
    ...
    print("Setting up schema permissions...")
        ...
        print(f"ERROR: Failed to create EDS user: {e}")
        return False
...
def main():
    ...
    if not create_eds_user():
        print(
            "\nERROR: Failed to create EDS user. Please check your PostgreSQL connection and try again."
        )
        return False
    ...
    if not test_eds_user_connection():
        print(
            "\nWARNING: EDS user was created but connection test failed. Check permissions."
        )
        return False
    ...
    if not update_env_file():
        print(
            "\nWARNING: EDS user works but failed to update .env file. Please update manually."
        )
        return False
