...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("logs/nvms_import.log"), logging.StreamHandler()],
)
        ...
        for idx, row in df.iterrows():
            try:
                ...
                if not start_date_dt or not end_date_dt:
                    logger.warning(
                        f"Row {idx}: Invalid date information for tile {tile_id}"
                    )
                    error_count += 1
                    continue
                ...
                if existing_result:
                    ...
                    existing_result.visual_check = clean_string_value(
                        row.get("visual_check")
                    )
                    ...
                    existing_result.cleared = (
                        int(row["cleared"]) if pd.notna(row.get("cleared")) else None
                    )
                    existing_result.not_cleared = (
                        int(row["not_cleared"])
                        if pd.notna(row.get("not_cleared"))
                        else None
                    )
                    existing_result.supplied_to_ceb = clean_string_value(
                        row.get("supplied_to_ceb")
                    )
                    existing_result.start_date = clean_string_value(
                        row.get("start_date")
                    )
                ...
                if not existing_history:
                    # Calculate totals
                    cleared_count = (
                        int(row["cleared"]) if pd.notna(row.get("cleared")) else 0
                    )
                    not_cleared_count = (
                        int(row["not_cleared"])
                        if pd.notna(row.get("not_cleared"))
...
def generate_summary_report(db_manager):
    ...
    with db_manager.get_session() as session:
        ...
        for run in runs:
            ...
            tiles_with_clearing = len(
                [r for r in results if r.cleared and r.cleared > 0]
            )
            ...
            if paths and rows:
                print(
                    f"  Path coverage: {min(paths)}-{max(paths)} ({len(paths)} paths)"
                )
...
def main():
    ...
    try:
        ...
        runs_to_import = [
            {
                "csv_path": pilot_data_dir
                / "NVMS_QLD_Run01_Processed_Record_clean.csv",
                "run_id": "NVMS_QLD_Run01",
                "run_number": 1,
            },
            {
                "csv_path": pilot_data_dir
                / "NVMS_QLD_Run02_Processed_Record_clean.csv",
                "run_id": "NVMS_QLD_Run02",
                "run_number": 2,
            },
            {
                "csv_path": pilot_data_dir
                / "NVMS_QLD_Run03_Processed_Record_clean.csv",
