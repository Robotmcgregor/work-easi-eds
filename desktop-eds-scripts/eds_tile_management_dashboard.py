    ...
    app.layout = html.Div(
        [
            ...
            dcc.Tabs(
                ...
                value="tiles-tab",
                children=[
                    dcc.Tab(label="Tiles Overview", value="tiles-tab"),
    ...
    def handle_actions(
        ...
        if button_id == "update-tiles-btn" and update_clicks:
            if not selected_tiles_json:
                return html.Div("No tiles selected", style={"color": "orange"})

            selected_tiles = json.loads(selected_tiles_json)
            if not staff_member or not new_status:
                return html.Div(
                    "Please select staff member and status", style={"color": "orange"}
                )
            ...
            if not tile_id or not start_date or not end_date or not script:
                return html.Div(
                    "Please fill all script parameters", style={"color": "orange"}
                )
