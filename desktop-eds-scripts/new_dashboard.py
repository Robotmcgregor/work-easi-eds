...
if __name__ == "__main__":
    ...
    @app.callback(
        Output("nvms-runs-chart", "figure"), [Input("refresh-interval", "n_intervals")]
    )
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                fig = px.bar(
                    df,
                    x="Run",
                    y=["Tiles Processed", "Detections Found"],
    ...
    @app.callback(
        Output("timeline-chart", "figure"), [Input("refresh-interval", "n_intervals")]
    )
    ...
    def update_main_map(n_intervals, map_style, show_detections):
        ...
        try:
                    ...
                    for tile in run_tiles[:50]:  # Limit for performance
                        if tile.bounds_geojson:
                            try:
                                ...
                                logger.error(
                                    f"Error adding boundary for {tile.tile_id}: {e}"
                                )
                    ...
                    if not added_boundaries and run_tiles:
                        ...
                        fig.add_trace(
                            go.Scattermapbox(
                                ...
                                mode="markers",
                                marker=dict(
                                    size=6, color=run_colors[run_type], opacity=0.7
                                ),
                                text=tile_ids,
                                name=run_type,
            ...
            return go.Figure().add_annotation(
                text=f"Map Error: {str(e)}", xref="paper", yref="paper", x=0.5, y=0.5
            )
    ...
    print("- Street/terrain map options")
