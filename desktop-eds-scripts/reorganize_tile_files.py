...
def create_correct_filename(
    original_filename: str, actual_path: int, actual_row: int
) -> str:
    ...
    if "_fc3ms.tif" in original_filename:
        ...
        return re.sub(
            r"(\d{6})", f"{actual_path:03d}{actual_row:03d}", original_filename
        )


def ensure_directory_structure(
    base_path: Path, path: int, row: int, year: str, yearmonth: str
) -> Path:
...
def find_corresponding_files(fc_file: Path, base_dir: Path) -> list:
    ...
    for file_path in fc_file.parent.rglob("*"):
        if file_path.is_file() and date in file_path.name and file_path != fc_file:
            ...
            if any(
                suffix in file_path.name
                for suffix in ["_fmask.tif", "_clr.tif", "_oa.tif"]
            ):
...
def main():
    ...
    for result in analysis_results:
        try:
            ...
            dest_dir = ensure_directory_structure(
                base_dir, actual_path, actual_row, year, yearmonth
            )
            ...
            for corr_file in result["corresponding_files"]:
                try:
                    new_corr_name = create_correct_filename(
                        corr_file.name, actual_path, actual_row
                    )
    ...
    for tile_dir in sorted(base_dir.glob("???_???")):
        if tile_dir.is_dir():
            ...
            if fc_count > 0 or fmask_count > 0:
                print(
                    f"  {tile_dir.name}: {fc_count} FC files, {fmask_count} FMASK files"
                )
