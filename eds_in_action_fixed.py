...
if __name__ == "__main__":
    ...
    app.layout = html.Div([
        # Header
        html.Div([
            html.H1("🛰️ EDS IN ACTION - WORKING VERSION", 
                   style={'textAlign': 'center', 'color': '#2c3e50', 'marginBottom': 10}),
            html.P("Early Detection System with Quality Control", 
                  style={'textAlign': 'center', 'color': '#7f8c8d', 'marginBottom': 30}),
            html.Div([
                html.Small(f"Build: {BUILD_STAMP} | Instance: {INSTANCE_ID} | Port: {PORT} | Add ?v={INSTANCE_ID} to URL to bypass cache",
                           style={'color': '#7f8c8d'})
        ...
        html.Div([
            html.Div([
                html.Div([
                    html.H2(id="basic-tiles-count", children="...", style={'margin': 0, 'color': '#2c3e50'}),
            ...
            html.Div([
                html.Div([
                    html.H2(id="basic-detections-count", children="...", style={'margin': 0, 'color': '#2c3e50'}),
            ...
            html.Div([
                html.Div([
                    html.H2(id="qc-pending-count", children="...", style={'margin': 0, 'color': '#e74c3c'}),
                    html.P("Pending QC", style={'margin': '5px 0 0 0', 'color': '#7f8c8d'})
            ...
            html.Div([
                html.Div([
                    html.H2(id="qc-confirmed-count", children="...", style={'margin': 0, 'color': '#27ae60'}),
                    html.P("Confirmed", style={'margin': '5px 0 0 0', 'color': '#7f8c8d'})
        ...
        html.Div([
            ...
            html.Div([
                html.Label("Layers:", style={'marginRight': 10, 'fontWeight': 'bold'}),
                dcc.Checklist(
                    id='layer-toggles',
                    options=[
        ...
        ], style={'marginTop': 20, 'padding': 15, 'backgroundColor': '#f1f3f5', 'borderRadius': 8, 'border': '1px solid #dee2e6'}),
        
        # QC Section
        html.Div([
            html.H2("🔍 Quality Control Review", style={'color': '#2c3e50', 'marginBottom': 20}),
            
            html.Div([
                html.Div([
                    html.H3("📋 Detection Review", style={'color': '#2c3e50'}),
                    ...
                    html.Div([
                        ...
                        dcc.Input(id='reviewer-input', type='text', placeholder='Your name...', 
                        ...
                        html.Label("Confidence Level (1-5):", style={'fontWeight': 'bold', 'marginTop': '15px'}),
                        html.Div([
                            dcc.Slider(
                        ...
                        ], style={'marginBottom': '20px', 'padding': '10px', 'border': '1px solid #ddd', 'borderRadius': '4px', 'backgroundColor': '#f8f9fa'}),
                        
                        html.Label("Comments:", style={'fontWeight': 'bold'}),
                        dcc.Textarea(id='comments-input', placeholder='Optional comments...',
                                   style={'width': '100%', 'height': 80, 'marginBottom': '10px'}),
                        
                        html.Button("💾 Submit Review", id="submit-btn", n_clicks=0,
                                   style={'backgroundColor': '#27ae60', 'color': 'white', 'border': 'none', 'padding': '10px 20px', 'borderRadius': '4px'}),
                        
                        html.Div(id="submit-message", style={'marginTop': 10})
                    ])
                    
                ], className="eight columns"),
                
                html.Div([
                    html.H3("📊 Recent Reviews", style={'color': '#2c3e50'}),
    ...
    def update_basic_counts(n_intervals):
        try:
            ...
            with db.get_session() as session:
                ...
                try:
                    pending = session.query(QCValidation).filter(QCValidation.qc_status == 'pending').count()
                    confirmed = session.query(QCValidation).filter(QCValidation.qc_status == 'confirmed').count()
                except:
                    pending = 0
                    confirmed = 0
    ...
    def update_map(map_style, layers, qld_clicks, aus_clicks, wide_clicks, _n):
        try:
            ...
            with db.get_session() as session:
                ...
                try:
                    nvms_last = (
                        session.query(NVMSDetection.tile_id, func.max(NVMSDetection.imported_at))
                ...
                try:
                    rows = session.execute(text("SELECT tile_id, MAX(detection_date) AS last_dt FROM detection_alerts GROUP BY tile_id"))
                    for row in rows:
                        tile_id = row[0]
                        dt = row[1]
                        if tile_id and dt:
                        ...
                        if tile.center_lat and tile.center_lon:
                            ...
                            if last_dt:
                                ...
                                if days_ago <= 7:
                                    ...
                                    color = 'red'
                                last_text = last_dt.strftime('%Y-%m-%d') if hasattr(last_dt, 'strftime') else str(last_dt)
                            else:
                                color = 'gray'
                                last_text = 'Never'

                            tile_colors.append(color)
                            tile_texts.append(f"Tile: {tile.tile_id}<br>Last Seen: {last_text}")

                    fig.add_trace(go.Scattermapbox(
                        lat=tile_lats,
                        lon=tile_lons,
                        mode='markers',
                        marker=dict(size=marker_size, color=tile_colors, opacity=0.55),
                        text=tile_texts,
                        name='EDS Tiles',
                ...
                try:
                    detections = session.query(NVMSDetection).order_by(NVMSDetection.id.desc()).limit(200).all()
                        ...
                        if getattr(det, 'geom_geojson', None):
                            try:
                                ...
                                if coords:
                                    ...
                                    det_date = det.imported_at.strftime('%Y-%m-%d') if getattr(det, 'imported_at', None) else 'Unknown'
                                    nvms_texts.append(f"NVMS #{det.id}<br>Tile: {det.tile_id}<br>Imported: {det_date}")
                            except Exception:
                                continue
                    if nvms_lats and ('nvms' in (layers or [])):
                        fig.add_trace(
                            go.Scattermapbox(
                                ...
                                lon=nvms_lons,
                                mode='markers',
                                marker=dict(size=11, color='red', symbol='circle', opacity=0.9),
                                text=nvms_texts,
                ...
                try:
                    rows = session.execute(text("SELECT id, tile_id, detection_date, detection_geojson FROM detection_alerts ORDER BY detection_date DESC LIMIT 300"))
                    ...
                    for row in rows:
                        ...
                        if center_lat is not None and center_lon is not None:
                            ...
                            date_text = det_date.strftime('%Y-%m-%d') if hasattr(det_date, 'strftime') else str(det_date)
                            alert_texts.append(f"Alert {alert_id}<br>Tile: {tile_id or 'Unknown'}<br>Date: {date_text}")
                    ...
                    if alert_lats and ('alerts' in (layers or [])):
                        fig.add_trace(
                            go.Scattermapbox(
                                ...
                                lon=alert_lons,
                                mode='markers',
                                marker=dict(size=12, color='orange', symbol='diamond', opacity=0.95),
                                text=alert_texts,
                ...
                fig.update_layout(
                    ...
                    legend=dict(title='Layers', orientation='h', yanchor='bottom', y=0.01, xanchor='left', x=0.01, bgcolor='rgba(255,255,255,0.7)')
                )
    ...
    def update_summary(_n):
        try:
            ...
            with db.get_session() as session:
                ...
                try:
                    nvms_with_geom = session.query(NVMSDetection).filter(NVMSDetection.geom_geojson.isnot(None)).count()
                except Exception:
                    nvms_with_geom = 0
                try:
                    alerts_total = session.execute(text("SELECT COUNT(*) FROM detection_alerts")).scalar() or 0
                    alerts_with_geo = session.execute(text("SELECT COUNT(*) FROM detection_alerts WHERE detection_geojson IS NOT NULL")).scalar() or 0
                except Exception:
                    alerts_total = 0
                    alerts_with_geo = 0
                ...
                def count_nvms_plot_points():
                    try:
                        c = 0
                        dets = session.query(NVMSDetection).order_by(NVMSDetection.id.desc()).limit(200).all()
                        for det in dets:
                            g = det.geom_geojson
                            if not g:
                                continue
                            ...
                            if isinstance(g, dict):
                                ...
                                if t == 'polygon':
                                    ...
                                    coords = (g.get('coordinates', [[[]]])[0][0]) if g.get('coordinates') else None
                            if coords:
                                c += 1
                        return c
                    except Exception:
                ...
                def count_alert_plot_points():
                    try:
                        rows = session.execute(text("SELECT detection_geojson FROM detection_alerts ORDER BY detection_date DESC LIMIT 300"))
                        c = 0
                        import json as _json
                        for (geojson,) in rows:
                            if not geojson:
                                continue
                            g = _json.loads(geojson) if isinstance(geojson, str) else geojson
                            if isinstance(g, dict):
                                t = (g.get('type') or '').lower()
                                if t == 'polygon':
                                    c += 1
                ...
                return html.Div([
                    html.Div([
                        html.Span(f"Tiles - green:{green} orange:{orange} red:{red} grey:{gray}")
                    ]),
                    html.Div([
                        html.Span(f"NVMS detections: {nvms_total} (with geometry: {nvms_with_geom}; plot candidates: {nvms_plot})")
                    ]),
                    html.Div([
                        html.Span(f"Alert detections: {alerts_total} (with geometry: {alerts_with_geo}; plot candidates: {alerts_plot})")
                    ])
    ...
    def show_detection_info(detection_id):
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                try:
                    ...
                    if raw:
                        ...
                        if ring:
                            ...
                            polygon_fig.add_trace(go.Scattermapbox(
                                lon=lons,
                                lat=lats,
                                mode='lines',
                                fill='toself',
                    ...
                    polygon_fig.update_layout(
                        height=180,
                        margin=dict(l=5, r=5, t=30, b=5),
                        title={'text': "⚠️ Geometry parse error", 'x': 0.5, 'xanchor': 'center'}
                    )
                    polygon_fig.add_annotation(
                ...
                return html.Div([
                    html.Div([
                        html.Strong(f"Detection #{detection.id}", style={'fontSize': '16px', 'color': '#2c3e50'}),
                        html.Br(),
                    ...
                    html.Div([
                        html.Strong("🗺️ Polygon Shape Preview:", style={'marginBottom': 10, 'display': 'block', 'fontSize': '16px'}),
                        dcc.Graph(
                            figure=polygon_fig,
                            config={'displayModeBar': False}
                        )
                    ], style={'border': '2px solid #3498db', 'borderRadius': '8px', 'padding': '10px', 'backgroundColor': '#f8f9ff', 'marginTop': '10px'})
                ])
    ...
    def submit_review(n_clicks, detection_id, reviewer, decision, confidence, comments):
        ...
        try:
            ...
            with db.get_session() as session:
                ...
                success_msg = html.Div(f"✅ Review submitted! Detection {detection_id} marked as {decision} (confidence: {confidence}/5)",
    ...
    def show_recent_reviews(n_intervals, submit_clicks):
        try:
            ...
            with db.get_session() as session:
                recent = session.query(QCValidation).order_by(QCValidation.reviewed_at.desc()).limit(5).all()
                ...
                items = []
                for qc in recent:
                    color = {'confirmed': '#27ae60', 'rejected': '#e74c3c', 'requires_review': '#f39c12'}.get(qc.qc_status, '#95a5a6')
                    ...
                    ], style={'padding': 8, 'marginBottom': 8, 'backgroundColor': '#f8f9fa', 'borderRadius': 4}))
