...
app.layout = html.Div([
    html.Div([
        ...
        html.P("Land Clearing Detection & Quality Control Dashboard", 
               style={'textAlign': 'center', 'color': '#7f8c8d', 'fontSize': 18}),
        html.Div([
            html.Small(f"Build: {BUILD_STAMP} | Instance: {INSTANCE_ID} | Port: 8060 | Add ?v={INSTANCE_ID} to URL to bypass cache",
                       style={'color': '#7f8c8d'})
    ...
    html.Div([
        ...
        ], className="twelve columns", style={'backgroundColor': '#f8f9fa', 'padding': 15, 'borderRadius': 5})
    ...
    html.Div([
        # Left column - Map
        html.Div([
            html.H3("🗺️ Landsat Tiles & Detections", style={'color': '#2c3e50'}),
            
            html.Div([
                html.Label("Map Style:", style={'fontWeight': 'bold', 'marginRight': 10}),
                dcc.RadioItems(
                    id='map-style-radio',
                    options=[
            ...
            html.Div([
                html.Strong("Legend: "),
                html.Span("🟢 Recent (≤7 days) ", style={'color': 'green'}),
                html.Span("🟠 Medium (8-30 days) ", style={'color': 'orange'}),
                html.Span("🔴 Old (>30 days) ", style={'color': 'red'}),
                html.Span("⚫ Never Run ", style={'color': 'gray'}),
                html.Br(),
                html.Span("🔴 Red circles = NVMS pilot detections ", style={'color': 'red', 'fontWeight': 'bold'}),
                html.Span("🟠 Orange diamonds = Live detection alerts", style={'color': 'orange', 'fontWeight': 'bold'})
            ], style={'marginTop': 10, 'padding': 10, 'backgroundColor': '#f8f9fa', 'borderRadius': 5})
        ...
        html.Div([
            ...
            # Detection selection
            html.Div([
                html.Label("Select Detection to Review:", style={'fontWeight': 'bold'}),
                dcc.Dropdown(
            ...
            # QC Review Form
            html.Div([
                html.H4("📝 Review Form", style={'color': '#34495e', 'marginBottom': 15}),
                
                html.Label("Reviewer Name:", style={'fontWeight': 'bold'}),
                dcc.Input(
                    id='reviewer-input', 
                    type='text', 
                ...
                dcc.RadioItems(
                    id='decision-radio',
                    options=[
                        {'label': ' ✅ Confirmed Clearing', 'value': 'confirmed'},
                        {'label': ' ❌ False Positive', 'value': 'rejected'},
                        {'label': ' ⚠️ Needs Review', 'value': 'requires_review'}
                    ],
                    style={'marginBottom': '20px'}
                ),
                
                html.Label("Confidence Level:", style={'fontWeight': 'bold'}),
                html.Div([
                    dcc.Slider(
                        id='confidence-slider',
                        min=1, max=5, step=1, value=3,
                        marks={1: '1-Low', 2: '2', 3: '3-Med', 4: '4', 5: '5-High'},
                ...
                dcc.Textarea(
                    ...
                    style={'width': '100%', 'height': 80, 'marginBottom': '15px', 'padding': '8px'}
                ),
...
def update_stats(n_intervals):
    try:
        with get_db_session() as session:
            ...
            recent_runs = session.query(LandsatTile).filter(
                LandsatTile.last_processed >= datetime.now() - timedelta(days=7)
            ).count()
            
            return html.Div([
                html.Div([
                    html.H4(f"{total_tiles:,}", style={'margin': 0, 'color': '#2980b9'}),
                    html.P("Total Tiles", style={'margin': 0})
                ], className="three columns", style={'textAlign': 'center'}),
                
                html.Div([
                    html.H4(f"{total_detections:,}", style={'margin': 0, 'color': '#e74c3c'}),
                    html.P(f"Detections ({nvms_detections} NVMS)", style={'margin': 0})
                ], className="three columns", style={'textAlign': 'center'}),
                
                html.Div([
                    html.H4(f"{recent_runs:,}", style={'margin': 0, 'color': '#27ae60'}),
        ...
        return html.Div([
            ...
            html.P("Check console for details", style={'color': 'gray', 'fontSize': '10px'})
        ])
            ...
            if tiles:
                ...
                for tile in tiles:
                    if tile.center_lat and tile.center_lon:
                        ...
                        last_run_str = tile.last_processed.strftime('%Y-%m-%d') if tile.last_processed else 'Never'
                        tile_texts.append(f"Tile: {tile.tile_id}<br>Last Run: {last_run_str}")
            ...
            if nvms_detections:
                    ...
                    if det.geom_geojson:
                        try:
                            ...
                            if geom['type'] == 'Polygon':
                                ...
                                det_texts.append(f"NVMS Detection #{det.id}<br>Tile: {det.tile_id}<br>Imported: {det.imported_at}")
                        except:
                            continue
                ...
                if det_lats:
                    fig.add_trace(go.Scattermapbox(
                        ...
                        lon=det_lons,
                        mode='markers',
                        marker=dict(size=6, color='red', symbol='circle', opacity=0.9),
                        text=det_texts,
            ...
            try:
                ...
                if alerts:
                    ...
                    fig.add_trace(go.Scattermapbox(
                        ...
                        lon=alert_lons,
                        mode='markers',
                        marker=dict(size=8, color='orange', symbol='diamond', opacity=0.9),
                        text=alert_texts,
        ...
        with get_db_session() as session:
            ...
            try:
                ...
                for alert in alerts:
                    ...
                    label = f"Alert {alert[0]} - Tile {alert[1]} ({date_str}, {area_str})"
...
def update_detection_info(detection_id):
    if not detection_id or detection_id == 'error':
        return html.Div("Select a detection to see details and polygon shape.", 
    ...
    try:
        with get_db_session() as session:
            ...
            if detection_id.startswith('nvms_'):
                ...
                detection = session.query(NVMSDetection).filter(NVMSDetection.id == nvms_id).first()
                ...
                return html.Div([
                    # Detection details
                    html.Div([
                        html.H4(f"🎯 NVMS Detection #{detection.id}", style={'color': '#2c3e50', 'marginBottom': 10}),
                        html.P([
                            html.Strong("Type: "), "NVMS Pilot Data", html.Br(),
                            html.Strong("Tile ID: "), f"{detection.tile_id}", html.Br(),
                            html.Strong("Run ID: "), f"{detection.run_id}", html.Br(),
                            html.Strong("Result ID: "), f"{detection.result_id}", html.Br(),
                            html.Strong("Imported: "), detection.imported_at.strftime('%Y-%m-%d %H:%M') if detection.imported_at else "Unknown"
                        ])
                    ], style={
                    ...
                    # Polygon preview
                    html.Div([
                        html.Strong("🗺️ Detection Shape Preview:", style={'display': 'block', 'marginBottom': '10px'}),
                        dcc.Graph(
                            figure=polygon_fig,
                ...
                if not alert:
                    return html.Div("Detection Alert not found.", style={'color': 'red'})
                ...
                if alert[7]:  # detection_geojson
                    try:
                        geom = alert[7] if isinstance(alert[7], dict) else json.loads(alert[7])
                        if geom['type'] == 'Polygon':
                            coords = geom['coordinates'][0]
                            lons = [coord[0] for coord in coords]
                            lats = [coord[1] for coord in coords]
                ...
                return html.Div([
                    # Detection details
                    html.Div([
                        html.H4(f"🚨 Detection Alert {alert[0]}", style={'color': '#2c3e50', 'marginBottom': 10}),
                        html.P([
                            html.Strong("Type: "), "Live Detection Alert", html.Br(),
                            html.Strong("Tile ID: "), f"{alert[1]}", html.Br(),
                            html.Strong("Detection Date: "), f"{alert[2]}", html.Br(),
                            html.Strong("Area: "), f"{alert[3]:.2f} hectares" if alert[3] else "N/A", html.Br(),
                            html.Strong("Confidence: "), f"{alert[4]:.2f}" if alert[4] else "N/A", html.Br(),
                            html.Strong("Clearing Type: "), f"{alert[8]}" if alert[8] else "Unknown", html.Br(),
                            html.Strong("Severity: "), f"{alert[9]}" if alert[9] else "Unknown"
                        ])
                    ], style={
    ...
    try:
        with get_db_session() as session:
            ...
            qc = QCValidation(
                ...
                reviewer_comments=f"[{detection_type.upper()}] {comments}" if comments else f"[{detection_type.upper()}]",
            ...
            ], style={'color': '#27ae60', 'backgroundColor': '#d5f4e6', 'padding': '10px', 'borderRadius': '5px'})
            
            return success_msg, "", None, 3, ""
            
    except Exception as e:
        error_msg = html.Div(f"❌ Error submitting review: {e}", 
                           style={'color': '#e74c3c', 'backgroundColor': '#fadbd8', 'padding': '10px', 'borderRadius': '5px'})
...
if __name__ == '__main__':
    ...
    def _version():
        ...
        payload = {"build": BUILD_STAMP, "instance": INSTANCE_ID, "port": 8060, "name": "eds_dashboard_complete"}
