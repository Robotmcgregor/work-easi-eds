...
class QCValidation(Base):
    ...
    nvms_detection_id = Column(Integer, ForeignKey('nvms_detections.id'), nullable=False, index=True)
    tile_id = Column(String(20), nullable=False, index=True)
    
    # QC Details
    qc_status = Column(String(20), nullable=False, default=QCStatus.PENDING.value, index=True)
    reviewed_by = Column(String(100), nullable=True)  # Staff member name/ID
    reviewed_at = Column(DateTime, nullable=True)
    
    # Validation results
    is_confirmed_clearing = Column(Boolean, nullable=True)  # True=confirmed, False=rejected, None=pending
...
class QCAuditLog(Base):
    ...
    # Reference
    qc_validation_id = Column(Integer, ForeignKey('qc_validations.id'), nullable=False, index=True)
    
    # Change details
    action = Column(String(50), nullable=False)  # created, status_changed, comments_added, etc.
