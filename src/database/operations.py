    ...
    def get_tiles_needing_processing(hours_since_last: int = 24) -> List[LandsatTile]:
        ...
        with get_db() as session:
            return (
                ...
                .order_by(LandsatTile.processing_priority.desc())
                .all()
            )
...
class JobManager:
    ...
    @staticmethod
    def create_job(
        tile_id: str, time_start: datetime, time_end: datetime
    ) -> ProcessingJob:
    ...
    def get_active_jobs() -> List[ProcessingJob]:
        ...
        with get_db() as session:
            return (
                session.query(ProcessingJob)
                .filter(ProcessingJob.status == ProcessingStatus.PROCESSING.value)
                .all()
            )
    ...
    def update_job_status(
        job_id: str, status: str, progress: int = None, error_message: str = None
    ) -> bool:
...
class AlertManager:
    ...
    def get_alerts_by_tile(tile_id: str, days_back: int = 30) -> List[DetectionAlert]:
        ...
        with get_db() as session:
            return (
                ...
                .order_by(desc(DetectionAlert.detection_date))
                .all()
            )
    ...
    def get_recent_alerts(hours_back: int = 24) -> List[DetectionAlert]:
        ...
        with get_db() as session:
            return (
                session.query(DetectionAlert)
                .filter(DetectionAlert.created_at >= cutoff_time)
                .order_by(desc(DetectionAlert.detection_date))
                .all()
            )
    ...
    def update_alert_verification(
        alert_id: str, verification_status: str, verified_by: str, notes: str = None
    ) -> bool:
        ...
        try:
            with get_db() as session:
                alert = (
                    session.query(DetectionAlert)
                    .filter(DetectionAlert.alert_id == alert_id)
                    .first()
                )
                if alert:
...
class SystemStatusManager:
    ...
    def get_system_stats() -> Dict:
        ...
        with get_db() as session:
            ...
            tile_stats = (
                session.query(
                    LandsatTile.status, func.count(LandsatTile.id).label("count")
                )
                .filter(LandsatTile.is_active == True)
                .group_by(LandsatTile.status)
                .all()
            )
            ...
            alerts_today = (
                session.query(DetectionAlert)
                .filter(func.date(DetectionAlert.created_at) == today)
                .count()
            )
            ...
            jobs_today = (
                session.query(ProcessingJob)
                .filter(func.date(ProcessingJob.created_at) == today)
                .count()
            )
