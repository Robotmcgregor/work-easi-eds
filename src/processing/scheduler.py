...
class EDSScheduler:
    ...
    def _run_daily_processing(self):
        ...
        try:
            ...
            logger.info(f"Daily processing complete: {successful}/{len(results)} tiles successful, "
    ...
    def _run_urgent_processing(self):
        ...
        try:
            ...
            if results:
                successful = sum(1 for r in results if r.success)
                logger.info(f"Urgent processing complete: {successful}/{len(results)} tiles processed")
    ...
    def _run_weekly_full_processing(self):
        ...
        try:
            ...
            config = EDSPipelineManager.create_processing_config(
                days_back=7,
                max_concurrent_jobs=6  # Use more resources for weekly run
            )
            results = EDSPipelineManager.run_automatic_processing(
                hours_since_last=168,  # 7 days
                config=config
            )
            ...
            logger.info(f"Weekly processing complete: {successful}/{len(results)} tiles successful, "
                       f"{total_alerts} alerts detected")
