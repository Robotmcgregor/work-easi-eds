...
DATE_IN_KEY = re.compile(r"((?:19|20)\d{2})[-_/]?(0[1-9]|1[0-2])[-_/]?([0-2][0-9]|3[01])")
...
class DataAccessor:
    def __init__(self,
        ...
        eff_bucket = bucket or (getattr(self.cfg, 's3', None).bucket if getattr(self.cfg, 's3', None) else None)
        eff_region = region or (self.cfg.s3.region if getattr(self.cfg, 's3', None) else None)
        eff_profile = profile or (self.cfg.s3.profile if getattr(self.cfg, 's3', None) else None)
        eff_endpoint = endpoint_url or (self.cfg.s3.endpoint_url if getattr(self.cfg, 's3', None) else None)
        eff_role = role_arn or (self.cfg.s3.role_arn if getattr(self.cfg, 's3', None) else None)
        self.base_prefix = base_prefix if base_prefix is not None else ((self.cfg.s3.base_prefix if getattr(self.cfg, 's3', None) else "") or "")
    ...
    def list_s3_for_tile(self, tile_id: str, start: date, end: date, prefix: Optional[str] = None, max_keys: int = 1000) -> List[str]:
        if not self.s3:
        ...
        if prefix is not None and (tile_id in prefix or (len(tile_id) == 6 and f"{tile_id[:3]}_{tile_id[3:]}" in prefix)):
            ...
            no_us = f"{tile_id}/"
            with_us = f"{tile_id[:3]}_{tile_id[3:]}/" if len(tile_id) == 6 and tile_id.isdigit() else tile_id + "/"
        ...
        for suffix in candidates:
            ...
            try:
                keys = list(self.s3.list_prefix(search_prefix, recursive=True, max_keys=max_keys))
                keys_all.extend(keys)
            except Exception as e:
                logger.warning(f"S3 listing failed for prefix '{search_prefix}': {e}. Trying next candidate (if any).")
                continue
        if not keys_all:
            return []
    ...
    def ensure_local_inputs(self, tile_id: str, start: date, end: date, prefix: Optional[str] = None, download: bool = True, max_files: Optional[int] = None) -> List[str]:
        ...
        # 1) Try S3 first
        keys = self.list_s3_for_tile(tile_id, start, end, prefix=prefix, max_keys=max_files or 2000)
        ...
        logger.info(f"No S3 objects found for tile {tile_id} in {start}..{end}. Attempting USGS M2M fetch...")

        # 2) Fallback to USGS M2M if configured
        m2m_paths: List[str] = []
        try:
            if getattr(self.cfg, 'usgs', None) and self.cfg.usgs.username and self.cfg.usgs.password:
                if USGSM2MClient is None:
                    raise RuntimeError("USGS M2M client not available.")
                # Convert tile_id 'PPP RRR' to path/row
                p = int(tile_id[:3])
                r = int(tile_id[3:6])
                client = USGSM2MClient(self.cfg.usgs.username, self.cfg.usgs.password, endpoint=self.cfg.usgs.endpoint)
                scenes = client.search_scenes_wrs2(self.cfg.usgs.dataset, p, r, start, end, node=self.cfg.usgs.node, max_results=max_files or 100)
                if scenes:
                    ...
                    if entity_ids:
                        options = client.get_download_options(self.cfg.usgs.dataset, entity_ids, node=self.cfg.usgs.node)
                            ...
                            if isinstance(opt, dict) and opt.get('entityId') and opt.get('products'):
                                for prod in opt['products']:
                                    if prod.get('available'):
                                        ...
                                        break
                        if products:
                            downloads = client.request_download(self.cfg.usgs.dataset, products, node=self.cfg.usgs.node)
                            m2m_paths = client.download_files(downloads, local_dir)
                            logger.info(f"Downloaded {len(m2m_paths)} file(s) from USGS M2M")
                            return m2m_paths
