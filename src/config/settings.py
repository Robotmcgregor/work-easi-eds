...
class USGSConfig:
    ...
    endpoint: str = os.environ.get(
        "USGS_M2M_ENDPOINT", "https://m2m.cr.usgs.gov/api/api/json/stable/"
    )
    dataset: str = os.environ.get(
        "USGS_DATASET", "LANDSAT_8_C2_L1"
    )  # Example; adjust per collection
...
class ConfigManager:
    ...
    def _load_from_environment(self) -> Dict[str, Any]:
        ...
        if "CONFIDENCE_THRESHOLD" in os.environ:
            processing_env["confidence_threshold"] = float(
                os.environ["CONFIDENCE_THRESHOLD"]
            )
        if "MAX_CONCURRENT_JOBS" in os.environ:
            processing_env["max_concurrent_jobs"] = int(
                os.environ["MAX_CONCURRENT_JOBS"]
            )
        ...
        if "ENABLE_SCHEDULER" in os.environ:
            processing_env["enable_scheduler"] = (
                os.environ["ENABLE_SCHEDULER"].lower() == "true"
            )
    ...
    def validate_config(self) -> bool:
        ...
        try:
            ...
            if (
                self._config.processing.confidence_threshold < 0
                or self._config.processing.confidence_threshold > 1
            ):
