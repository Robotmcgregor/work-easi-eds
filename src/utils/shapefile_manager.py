...
class ShapefileManager:
    ...
    def _extract_path_row(self, row) -> Tuple[Optional[int], Optional[int]]:
        ...
        if (path is None or row_num is None) and 'Name' in row.index and pd.notna(row['Name']):
            name = str(row['Name'])
        ...
        if (path is None or row_num is None) and 'PopupInfo' in row.index and pd.notna(row['PopupInfo']):
            popup_info = str(row['PopupInfo'])
            
            # Look for PATH in the HTML content
            import re
            path_match = re.search(r'<strong>PATH</strong>:\s*(\d+(?:\.\d+)?)', popup_info)
            row_match = re.search(r'<strong>ROW</strong>:\s*(\d+(?:\.\d+)?)', popup_info)
    ...
    def get_tiles_in_bounds(self, min_lat: float, max_lat: float, 
