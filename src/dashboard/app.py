            ...
            [
                html.Div(
                    [
                        ...
                        html.Button(
                            "Close", id="close-modal-btn", className="btn-secondary"
                        ),
                    ],
...
@app.callback(
    [Output("tile-data-store", "data"), Output("alert-data-store", "data")],
            ...
            tile_data.append(
                {
                    ...
                    "last_processed": (
                        tile.last_processed.isoformat() if tile.last_processed else None
                    ),
...
@app.callback(
    ...
    [Input("tile-data-store", "data"), Input("alert-data-store", "data")],
)
...
@app.callback(Output("tile-status-chart", "figure"), [Input("tile-data-store", "data")])
...
@app.callback(Output("tile-map", "figure"), [Input("tile-data-store", "data")])
def update_tile_map(tile_data):
    ...
    try:
        ...
        try:
                    ...
                    if detections:
                        ...
                        for det in detections:
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Error converting geometry JSON for detection {getattr(det,'id',None)}: {e}"
                                )
                                continue
            ...
            tile_fig.update_layout(
                title="Landsat Tiles & NVMS Detections (OpenStreetMap)"
            )
            return tile_fig
