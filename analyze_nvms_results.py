...
def analyze_processing_timeline():
    ...
    with db_manager.get_session() as session:
        ...
        for result in results:
            ...
            if tile_id not in tile_last_processed or run_date > tile_last_processed[tile_id]['date']:
        ...
        unprocessed_tiles = [tile for tile in all_tiles if tile.tile_id not in processed_tile_ids]
...
def analyze_detection_patterns():
    ...
    with db_manager.get_session() as session:
        ...
        results = session.query(NVMSResult).filter(
            (NVMSResult.cleared != None) | (NVMSResult.not_cleared != None)
        ).all()
        ...
        for run_id, stats in run_stats.items():
            ...
            if stats['high_clearing_tiles']:
                ...
                for tile_id, count, analyst in sorted(stats['high_clearing_tiles'], 
    ...
    with db_manager.get_session() as session:
        ...
        for result in results:
            ...
            if analyst not in analyst_stats:
                analyst_stats[analyst] = {'total_tiles': 0, 'runs': set(), 'tile_breakdown': {}}
        ...
        for analyst, stats in sorted(analyst_stats.items(), 
                                   key=lambda x: x[1]['total_tiles'], reverse=True):
            print(f"\n{analyst}:")
            print(f"  Total tiles processed: {stats['total_tiles']}")
            print(f"  Runs participated: {len(stats['runs'])} ({', '.join(sorted(stats['runs']))})")
...
def find_tile_processing_history(tile_id):
    ...
    with db_manager.get_session() as session:
        ...
        ).order_by(NVMSResult.start_date_dt).all()
        ...
        if nvms_results:
            ...
            for result in nvms_results:
                ...
                print(f"\n  {result.run_id}:")
                print(f"    Period: {result.start_date_dt.strftime('%Y-%m-%d')} to {result.end_date_dt.strftime('%Y-%m-%d')}")
                print(f"    Analyst: {result.analyst or 'Unknown'}")
                print(f"    Visual check: {result.visual_check}")
                print(f"    Detections: {total_detections} total ({result.cleared or 0} cleared, {result.not_cleared or 0} not cleared)")
                if result.comments:
        ...
        proc_history = session.query(ProcessingHistory).filter(
            ProcessingHistory.tile_id == tile_id
        ).order_by(ProcessingHistory.processed_at).all()
...
def main():
    ...
    with db_manager.get_session() as session:
        ...
        high_activity_tile = session.query(NVMSResult).filter(
            NVMSResult.cleared > 50
        ).first()
