    ...
    with db_manager.get_session() as session:
        ...
        for shp_folder in shapefile_dir.iterdir():
            ...
            tile = (
                ...
                .filter(LandsatTile.tile_id == tile_info["tile_id"])
                .first()
            )
            ...
            nvms_result = (
                ...
                .filter(
                    ...
                    NVMSResult.run_id
                    == "NVMS_QLD_Run03",  # These shapefiles are from Run 3
                )
                .first()
            )
...
def verify_import():
    ...
    with db_manager.get_session() as session:
        ...
        for row in results:
            print(
                f"  {row[0]}: {row[1]} total ({row[2]} cleared, {row[3]} not cleared, avg {row[4]:.2f} ha)"
            )
