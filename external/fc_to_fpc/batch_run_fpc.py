...
# python
def main():
    p = argparse.ArgumentParser(description="Batch run `fpc_from_pv_fc.py` on all `*_fcm3s.tif` files")
    p.add_argument("--path", required=True, help="file or directory to scan for `*_fcm3s.tif`")
    p.add_argument("--script", default=os.path.join("fc_to_fpc", "fpc_from_pv_fc.py"),
    ...
    p.add_argument("--dry-run", action="store_true", help="only print commands, do not run")
    args = p.parse_args()
    ...
    print(f"Scanning `{args.path}` (recursive={args.recursive}) -> found {len(inputs)} file(s)")
    if not inputs:
