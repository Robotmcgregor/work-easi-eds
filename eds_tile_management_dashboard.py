...
if __name__ == "__main__":
        ...
        try:
            ...
            def monitor_process():
                ...
                try:
                    ...
                    active_processes[process_id]['status'] = 'completed' if return_code == 0 else 'failed'
                    active_processes[process_id]['end_time'] = datetime.now()
                    
                    # Update tile status
                    if return_code == 0:
                        update_tile_status(tile_id, 'completed', notes=f"Completed {script_name} at {datetime.now()}")
                    else:
                        update_tile_status(tile_id, 'failed', notes=f"Failed {script_name} at {datetime.now()}")
    ...
    app.layout = html.Div([
        # Header
        html.Div([
            html.H1("🗺️ EDS Tile Management Dashboard", style={'textAlign': 'center', 'color': '#2c3e50'}),
            html.P("Manage EDS processing workflow, assign tiles to staff, and execute scripts", 
        ...
        html.Div([
            html.Div([
                ...
                html.Button('Update Selected Tiles', id='update-tiles-btn', 
                           className='btn btn-primary', style={'marginBottom': '10px'})
            ], style={'width': '30%', 'display': 'inline-block', 'verticalAlign': 'top', 'padding': '20px'}),
            
            html.Div([
                html.H3("🚀 Script Execution"),
                dcc.Input(id='tile-input', type='text', placeholder='Tile ID (e.g., 089_078)', 
                         style={'width': '100%', 'marginBottom': '10px'}),
                dcc.Input(id='start-date-input', type='text', placeholder='Start Date (YYYYMMDD)', 
                         style={'width': '48%', 'display': 'inline-block', 'marginRight': '2%'}),
                dcc.Input(id='end-date-input', type='text', placeholder='End Date (YYYYMMDD)', 
                ...
                dcc.Dropdown(
                    ...
                    options=[
                        ...
                        {'label': '⚙️ EDS Processing', 'value': 'eds_processing'}
                    ],
                    placeholder="Select script to run",
                    style={'marginTop': '10px', 'marginBottom': '10px'}
                ),
                html.Button('🚀 Run Script', id='run-script-btn', className='btn btn-success')
            ], style={'width': '30%', 'display': 'inline-block', 'verticalAlign': 'top', 'padding': '20px'}),
        ...
        # Main content tabs
        dcc.Tabs(id='main-tabs', value='tiles-tab', children=[
        ...
        # Status messages
        html.Div(id='status-message', style={'padding': '10px', 'textAlign': 'center'}),
    ...
    def render_processes_tab():
        ...
        return html.Div([
            ...
            html.Div([
                html.H4("Process Logs"),
                html.Div(id='process-logs')
    ...
    def render_analytics_tab():
        ...
        if 'assigned_to' in df.columns:
            ...
            assigned_fig = px.bar(x=assigned_counts.index, y=assigned_counts.values,
    ...
    def update_stats_panel(n):
        ...
        in_progress = len(df[df['status'].isin(['processing', 'data_prep'])]) if 'status' in df.columns else 0
    ...
    @app.callback(
        ...
        [Input('update-tiles-btn', 'n_clicks'),
    ...
    def handle_actions(update_clicks, run_clicks, selected_tiles_json, 
                      staff_member, new_status, tile_id, start_date, end_date, script):
        ...
        if button_id == 'update-tiles-btn' and update_clicks:
            ...
            if not staff_member or not new_status:
                return html.Div("⚠️ Please select staff member and status", style={'color': 'orange'})
            ...
            return html.Div(f"✅ Updated {updated_count} tiles", style={'color': 'green'})
        
        elif button_id == 'run-script-btn' and run_clicks:
            if not tile_id or not start_date or not end_date or not script:
                return html.Div("⚠️ Please fill all script parameters", style={'color': 'orange'})
            
            process_id, message = run_eds_script(script, tile_id, start_date, end_date)
            
            if process_id:
                update_tile_status(tile_id, 'processing', notes=f"Started {script} at {datetime.now()}")
                return html.Div(f"🚀 {message} (ID: {process_id})", style={'color': 'green'})
            else:
    ...
    def update_process_logs(n):
        ...
        for pid, logs in process_logs.items():
            if logs:  # Only show if there are logs
                ...
                log_text = "\n".join([f"[{log['timestamp']}] {log['message']}" for log in recent_logs])
