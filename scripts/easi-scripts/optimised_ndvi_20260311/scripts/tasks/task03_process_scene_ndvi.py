...
def process_scene_to_s3(
    ...
    # ndvi_local = work_dir / f"lztmre_{tile}_{date}_ndvi_{int(epsg)}.tif"
    # fmk_local  = work_dir / f"lztmre_{tile}_{date}_ffmask_{int(epsg)}.tif"
    # print(f"[DEBUG] -- ndvi_local: {ndvi_local}")
    # print(f"[DEBUG] -- fmk_local:  {fmk_local}")

    # local staging filenames
    ndvi_local = work_dir / f"sl{platform[1:]}olre_{tile}_{date}_ga1-clr_e{int(epsg)}.tif"
    fmk_local  = work_dir / f"sl{platform[1:]}olre_{tile}_{date}_ga2_e{int(epsg)}.tif"
    print(f"[DEBUG] -- ndvi_local: {ndvi_local}")
    print(f"[DEBUG] -- fmk_local:  {fmk_local}")
