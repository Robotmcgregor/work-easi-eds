...
def main():
    ...
    if args.force_clr:
        args.sr_only_clr = True
        args.fc_only_clr = True
        args.fc_prefer_clr = True  # keeps behaviour consistent
    ...
    # ----------------------------------------------------------------------
    # Resolve DLL/DLJ outputs (support both legacy naming and vi-fpc naming)
    # ----------------------------------------------------------------------
    ...
    def _pick_existing(*candidates: Path) -> Path:
        for p in candidates:
            if p.exists():
                return p
        # In --dry-run nothing exists yet; return first expected candidate
        return candidates[0]
