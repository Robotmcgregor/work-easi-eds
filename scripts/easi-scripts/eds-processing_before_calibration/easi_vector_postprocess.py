...
def _create_out_layer(
    driver: ogr.Driver,
    out_path: Path,
    srs: osr.SpatialReference,
    template_lyr: ogr.Layer,
...
def _compute_area_fields(
    geom: ogr.Geometry, srs: osr.SpatialReference
) -> Tuple[float, float]:
...
def skinny_filter_and_dissolve(
    in_path: Path,
    out_path: Path,
    skinny_pixels: int,
    ...
    if skinny_pixels and skinny_pixels > 0:
        if pixel_size is None:
            raise SystemExit(
                "--skinny-pixels requires either --pixel-size or --from-raster"
            )
        core_buf_m = (skinny_pixels * pixel_size) / 2.0
        print(
            f"[INFO] Skinny-core filter: N={skinny_pixels} px -> in-buffer distance={core_buf_m:.3f} m"
        )
    ...
    attr_name = (
        "thr"
        if defn.GetFieldIndex("thr") != -1
        else ("class" if defn.GetFieldIndex("class") != -1 else None)
    )
    attr_val = None
    ...
    from_ae = None
    if needs_reproj:
        ae = osr.SpatialReference()
        ae.ImportFromEPSG(3577)
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Post-process clearing polygons: dissolve and/or skinny-core filter."
    )
    ...
    ap.add_argument(
        "--out-dir", required=True, help="Output directory for processed shapefiles"
    )
    ap.add_argument(
        "--dissolve", action="store_true", help="Dissolve adjacent/overlapping polygons"
    )
    ap.add_argument(
        "--skinny-pixels",
        type=int,
        default=0,
        help="Minimum core width in pixels (0 disables)",
    )
    ap.add_argument(
        "--pixel-size",
        type=float,
        default=None,
        help="Pixel size in layer CRS units (e.g., meters)",
    )
    ap.add_argument(
        "--from-raster",
        type=str,
        default=None,
        help="Raster to read pixel size from (uses geotransform)",
    )
    ...
    for shp in sorted(in_files):
        ...
        skinny_filter_and_dissolve(
            shp, out_path, args.skinny_pixels, pixel_size, args.dissolve
        )
