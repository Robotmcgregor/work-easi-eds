...
def derive_target_epsg_wgs84_utm_from_lonlat(lon: float, lat: float) -> int:
    ...
    # Clamp to valid UTM zones
    zone = max(1, min(60, zone))

    # Australia => lat < 0 => EPSG:327xx
    if lat < 0:
        return 32700 + zone
    return 32600 + zone


# Back-compat wrapper (keep this name if lots of code calls it)
# NOTE: It now returns WGS84 UTM (326/327), not GDA94 MGA (283).
