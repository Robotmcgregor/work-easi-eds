...
def run_download_from_comparison(
    ...
    """
    if not comparison_csv.exists():
        print(f"[DOWNLOAD] Comparison CSV not found, skipping download: {comparison_csv}")
        return
...
def main():
    ...
    parser.add_argument(
        ...
        help=(
            "Path to ls89_download_fc_sr_from_comparison.py. "
            f"Default: {DEFAULT_DOWNLOAD_SCRIPT}"
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "If set, after building comparison tables (and optional season filter) "
            "call ls89_download_fc_sr_from_comparison.py for each tile."
        ),
    )

    # (Optional) if you want to filter download by date window later:
    parser.add_argument(
        "--download-start-date",
        type=str,
        default=None,
        help="Optional start date 'YYYY-MM-DD' for the download script.",
    )
    parser.add_argument(
        "--download-end-date",
        type=str,
        default=None,
        help="Optional end date 'YYYY-MM-DD' for the download script.",
    )
    ...
    download_script_path = Path(args.download_script).expanduser()
    ...
    if args.run_download and not download_script_path.exists():
        raise FileNotFoundError(f"Download script not found: {download_script_path}")
    ...
    for idx, row in run_tiles.iterrows():
        ...
        # -----------------------------------------------------------------
        # NEW: trigger download/composite from comparison table
        # -----------------------------------------------------------------
        if args.run_download:
            # Prefer season-filtered table if it exists, otherwise full table
            if tile_comp_season is not None and tile_comp_season.exists():
                comp_for_download = tile_comp_season
                print(f"[PIPELINE] Using season table for download: {comp_for_download}")
            elif tile_comp is not None and tile_comp.exists():
                comp_for_download = tile_comp
                print(f"[PIPELINE] Using full table for download: {comp_for_download}")
            else:
