...
def build_presence_mask(
    ...
    xsize_ref = ref.RasterXSize
    ysize_ref = ref.RasterYSize
    nodatas = []
    for ds in datasets:
        band = ds.GetRasterBand(1)
    ...
    for src_path, ds, nd in zip(rasters, datasets, nodatas):
        ...
        if (
            (gt != gt_ref)
            or (ds.RasterXSize != xsize_ref)
            or (ds.RasterYSize != ysize_ref)
        ):
            # Warp to reference grid
            mem_drv = gdal.GetDriverByName("MEM")
            tmp = mem_drv.Create("", xsize_ref, ysize_ref, 1, gdal.GDT_Float32)
            tmp.SetGeoTransform(gt_ref)
            tmp.SetProjection(proj)
            gdal.ReprojectImage(
                ds, tmp, ds.GetProjection(), proj, gdal.GRA_NearestNeighbour
            )
    ...
    if shp_ds is None:
        ...
        if shp_ds is None:
            raise RuntimeError(
                f"Failed to create ratio mask shapefile: {shp_path} (and fallback {alt_path})"
            )
        shp_path = alt_path
    srs = osr.SpatialReference()
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Compute FC input union and consistent coverage polygons."
    )
    ap.add_argument("--fc-dir", required=True, help="Directory containing FC rasters")
    ap.add_argument(
        "--scene",
        required=True,
        help="Scene identifier (e.g., p089r080) used in output names",
    )
    ap.add_argument(
        "--pattern",
        default="*_dc4mz.img",
        help="Glob pattern for FC rasters (default *_dc4mz.img)",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=None,
        help="(Deprecated in multi-ratio mode) Single ratio; prefer --ratios",
    )
    ap.add_argument(
        "--ratios",
        type=float,
        nargs="*",
        default=None,
        help="One or more presence ratios (e.g. 0.95 0.90) to generate multiple coverage masks",
    )
    ap.add_argument(
        "--save-per-input-masks",
        action="store_true",
        help="Write per-input valid masks aligned to the reference grid",
    )
    ap.add_argument(
        "--per-input-dir",
        default=None,
        help="Directory to write per-input masks (default: <out-dir>/masks)",
    )
    ap.add_argument(
        "--force",
        action="store_true",
        help="Force overwrite of outputs (mask TIFF and shapefile) if they exist",
    )
    ...
    if not ratios:
        ...
        if args.save_per_input_masks:
            per_dir = (
                Path(args.per_input_dir) if args.per_input_dir else (out_dir / "masks")
            )
