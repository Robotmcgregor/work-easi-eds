...
# lzolre_p089r078_d2023030620231024_dlwm6
...
# lzolre_p089r084_d2023012520231024_dlwm6
...
# lzolre_p100r082_d2023042120231022_dlwm3
...
# lzolre_p104r074_d2023050320231026_dlwm2
...
# lzolre_p108r081_d2023031220231014_dlwm1
...
def run_cmd(
    ...
    step: str,
    results: dict,
    cwd: Optional[str] = None,   # NEW: optional working directory
) -> None:
    """
    ...
    #   - cwd: controls *where files are written* (critical for legacy scripts)
    ...
    results.setdefault("steps", []).append(
        {
            ...
            "cwd": cwd,                   # record where it was run
...
def _resolve_sr_input(
    ...
    # ----------------------------------------
    # 7b. If everything failed: stop clearly (non-clr case too)
    # ----------------------------------------
    raise SystemExit(
        "\n".join(
            [
                f"[ERR] No SR composite found for {tile} date {date_tag}.",
                f" sr_only_clr={sr_only_clr}",
                " Checked hint path (file/dir) and searched roots with multiple layouts/patterns.",
                f" Searched roots: {roots or 'N/A'}",
                " Tip: print a few real filenames and adjust patterns if they differ from nbart6m/srb6/srb7 expectations.",
            ]
        )
    )
...
def main():
    ...
    ap.add_argument(
        "--min-ha", type=float, default=10.0, help="Minimum polygon size in hectares"
    )
    ap.add_argument(
        "--skinny-pixels",
        type=int,
    ...
    ap.add_argument(
        "--diagnostics",
        action="store_true",
        help="Enable extra diagnostic outputs in the legacy method step",
    )
    ...
    if need_compat:
        ...
        if args.timeseries_source in ("fc", "fpc"):
            ...
            for fp in sr_files:
                ...
                if baseline_start_date <= d <= baseline_end_date:
                    sr_items.append((d, fp))
            ...
            print(f"[DEBUG] Produced dc4 files count: {len(dc4_existing)}")
    ...
    results["outputs"]["compat"].update({
        ...
        "dc4_existing": [str(p) for p in sorted(_glob.glob(dc4_glob))], 
    })
    ...
    if args.timeseries_source in ("fc", "fpc"):
        ...
        legacy_script = Path(__file__).resolve().parent / "easi_eds_legacy_method_window_sr_ndvi_calibrate40_80.py"
        #legacy_script = Path(__file__).resolve().parent / "easi_eds_legacy_method_window_sr_ndvi.py" - uses FC calibration thresholds
    else:
        raise ValueError(f"Unknown timeseries_source: {args.timeseries_source}")


    # ----------------------------------------------------------------------
    # Define VI tag used for output naming (must exist before cmd_legacy)
    # ----------------------------------------------------------------------
    vi_tag = {
        "fc": "vi-fc",
        "fpc": "vi-fpc",
        "ndvi": "vi-ndvi",
    }.get(args.timeseries_source)
    ...
    if getattr(args, "diagnostics", False):
        cmd_legacy.append("--diagnostics")


    # cmd_legacy.extend(["--vi-tag", vi_tag])
    # Only FC/FPC legacy supports --vi-tag
    if args.timeseries_source in ("fc", "fpc"):
        cmd_legacy.extend(["--vi-tag", vi_tag])
    ...
    vi_tag = {
        "fc": "vi-fc",     # legacy FC path uses FPC index naming
        "fpc": "vi-fpc",
        "ndvi": "vi-ndvi",
        }.get(args.timeseries_source, "vi-fc")
    ...
    # Resolve the DLL (clearing classification) raster
    dll = _pick_existing(
        compat_dir / f"lztmre_{scene}_d{eff_start}{eff_end}_{vi_tag}_dllmz.img",
        compat_dir / f"lztmre_{scene}_d{eff_start}{eff_end}_dllmz.img",
        label="DLL"
    )
    ...
    # Resolve the DLJ (interpretation / confidence) raster
    dlj = _pick_existing(
        compat_dir / f"lztmre_{scene}_d{eff_start}{eff_end}_{vi_tag}_dljmz.img",
        compat_dir / f"lztmre_{scene}_d{eff_start}{eff_end}_dljmz.img",
        label="DLJ"
    )
    ...
    # 15. Final summary
    # ----------------------------------------------------------------------
    results.setdefault("outputs", {}).update({
        "dll": str(dll),
        "dlj": str(dlj),
