...
def run_script(script_name, args_list, step_description, required=True):
    ...
    try:
        ...
        elapsed = time.time() - start_time
        log_step("ERROR", f"{step_description} failed after {elapsed:.1f}s (exit code {e.returncode})")
...
def calculate_seasonal_years(start_date_str, end_date_str, span_years, fc_end_year_override=None):
...
def check_existing_data(tile, dest_dir):
    ...
    sr_files = [
        ...
        if ("_srb" in f.name and (f.name.startswith("ga_ls8c_ard_") or f.name.startswith("ga_ls9c_ard_")))
    ]
    fc_files = [f for f in existing_files if "_fc3ms.tif" in f.name and "_clr" not in f.name]
    ...
    log_step("INVENTORY", f"SR: {len(sr_files)}, FC: {len(fc_files)}, FC_CLR: {len(fc_clr_files)}, FMASK: {len(fmask_files)}")
...
Examples:
  ...
  python eds_master_data_pipeline.py \\
    ...
    parser.add_argument("--span-years", type=int, default=10,
    ...
    parser.add_argument("--sr-root", help="Alias for --dest (logged if provided; --dest is used)")
    parser.add_argument("--fc-root", help="Alias for --dest (logged if provided; --dest is used)")
    parser.add_argument("--fc-end-year", help="Override computed FC end year (defaults to min(current_year, end_year+1))")
    ...
    log_step("PARAMS", f"Span: {args.span_years} years, Cloud: {args.cloud_cover}%, Search: ±{args.search_days} days")
    ...
    if args.sr_root and args.sr_root != args.dest:
        log_step("INFO", f"--sr-root provided ({args.sr_root}) differs from --dest ({args.dest}); using --dest for storage.")
    if args.fc_root and args.fc_root != args.dest:
        log_step("INFO", f"--fc-root provided ({args.fc_root}) differs from --dest ({args.dest}); using --dest for storage.")

    # Calculate seasonal data range
    fc_start_year, fc_end_year = calculate_seasonal_years(args.start_date, args.end_date, args.span_years, args.fc_end_year)
    ...
    if not args.skip_s3:
        ...
        log_step("INFO", "S3 hydration now validates FC provenance via DEA STAC and will only download LS8/LS9-derived FC (L5/L7 items are skipped).")
        ...
        s3_args = [
            "--tile", args.tile,
            "--start-yyyymm", start_yyyymm,
            "--end-yyyymm", end_yyyymm, 
            "--span-years", str(args.span_years),
            "--dest", args.dest,
            "--no-base-prefix"
        ]
        
        s3_success = run_script("download_fc_from_s3.py", s3_args, 
                               "S3 FC download", required=False)
    ...
    if not args.skip_sr:
        ...
        sr_args = [
            ...
            "--cloud-threshold", str(args.cloud_cover)
        ]
        
        run_script("ensure_sr_dates_for_eds.py", sr_args,
                  "SR dates verification", required=True)
    else:
    ...
    if not args.skip_fc:
        ...
        if start_month <= end_month:
            ...
            seasonal_months = ([str(m) for m in range(start_month, 13)] +  # Oct, Nov, Dec
                              [str(m) for m in range(1, end_month + 1)])    # Jan, Feb, Mar
        ...
        log_step("CALCULATE", f"Seasonal months to download: {', '.join(seasonal_months)}")
        
        fc_args = [
            "--tile", args.tile,
            "--start-year", str(fc_start_year),
            "--end-year", str(fc_end_year),
            "--dest", args.dest,
            "--temp-dir", args.temp_dir,
            "--cloud-threshold", str(args.cloud_cover),
            "--seasonal-months"] + seasonal_months
        
        run_script("download_seasonal_fc_from_ga.py", fc_args,
                  f"GA FC download ({fc_start_year}-{fc_end_year})", required=True)
    else:
    ...
    if not args.skip_masking:
        ...
        run_script("download_fmask_for_fc.py", fmask_args,
                  "FMASK download for FC masking", required=True)
    ...
    if not args.skip_masking:
        ...
        mask_args = [
            ...
            "--search-days", str(args.search_days),
            "--clear-values", "1"
        ]
        ...
        if mask_script:
            run_script(mask_script, mask_args,
                      "Clear mask application", required=True)
        else:
            log_step("WARNING", "No masking script found - FC data will not have clear masks")
    ...
    if args.validate_fc_provenance and not args.dry_run:
        ...
        if not prov_out:
            ...
            prov_out = str(Path(__file__).resolve().parents[1] / "data" / f"fc_{args.tile}_provenance.csv")
        prov_args = [
            "--tile", args.tile,
            "--root", args.dest,
            "--out", prov_out,
            "--action", args.prov_action,
            "--search-days", str(args.prov_search_days),
        ]
        if args.prov_action == "move":
            prov_args += ["--quarantine-dir", args.prov_quarantine_dir]
        run_script("validate_fc_provenance.py", prov_args, "FC provenance validation", required=False)
