...
def main():
    parser = argparse.ArgumentParser(
        description="Download seasonal FC data from GA DEA for EDS processing"
    )
    parser.add_argument("--tile", required=True, help="Tile in format 089_078")
    parser.add_argument(
        "--start-year", type=int, default=2014, help="Start year (default: 2014)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dest",
        required=True,
        help="Final destination directory (e.g., D:\\data\\lsat)",
    )
    parser.add_argument(
        "--temp-dir",
        default=None,
        help="Temporary working directory (default: auto-generated)",
    )
    parser.add_argument(
        "--cloud-threshold",
        type=float,
    ...
    for year in range(args.start_year, args.end_year + 1):
        ...
        for time_range in time_ranges:
            ...
            for feature in data_fc["features"]:
                    ...
                    if asset_name in ["bs", "pv", "npv"]:
                        ...
                        for i, part in enumerate(url_parts):
                            if (
                                part.isdigit()
                                and len(part) == 3
                                and i + 1 < len(url_parts)
                                and url_parts[i + 1].isdigit()
                                and len(url_parts[i + 1]) == 3
                            ):
                ...
                if (
                    (platform in ALLOWED_SENSORS)
                    and (cloud_cover < args.cloud_threshold)
                    and (feature_pathrow == target_pathrow)
                ):
                    filtered_features.append(feature)
                elif platform not in ALLOWED_SENSORS:
                    print(
                        f"[SKIP] Unsupported platform: {platform} (allowed: {sorted(ALLOWED_SENSORS)})"
                    )
                elif feature_pathrow != target_pathrow:
                    print(
                        f"[SKIP] Wrong tile: {feature_pathrow} (wanted {target_pathrow})"
                    )

            print(
                f"After cloud filter (<{args.cloud_threshold}%): {len(filtered_features)} scenes"
            )
                ...
                try:
                    ...
                    if len(band_files) == len(FC_BANDS):
                        ...
                        if not fmask_path.exists():
                            ...
                            for sr_collection in sr_collections:
                                try:
                                    ...
                                    print(
                                        f"Error getting fmask from {sr_collection}: {e}"
                                    )
                                    continue

                            if not fmask_downloaded:
                                print(
                                    f"WARNING: Could not download fmask for {date_str}"
                                )
                                dates_downloaded.add(date_str)
                                continue
                        ...
                        if fmask_path.exists():
                            ...
                            print(
                                f"WARNING: Fmask not available for {date_str}, skipping clear mask"
                            )

                    else:
                        print(
                            f"Incomplete FC bands for {date_str} - got {len(band_files)}/{len(FC_BANDS)}"
                        )
