...
def _download_from_s3(
    s3: Any, prefixes: List[str], tile: str, date: str, dest_root: Path
) -> bool:
    found_sr = []
    found_fm = None
    for pfx in prefixes:
    ...
    if found_fm:
        try:
            s3.download(
                found_fm, str(dest_dir / os.path.basename(found_fm)), overwrite=False
            )
...
def main(argv=None) -> int:
    ...
    ap = argparse.ArgumentParser(
        description="Ensure SR+FM exist for a tile/date using S3 or GA"
    )
    ...
    ap.add_argument(
        "--bbox",
        help="Optional WGS84 bbox minx,miny,maxx,maxy for GA fallback (avoids shapefile dependency)",
    )
    ap.add_argument(
        "--search-days",
        type=int,
        default=0,
        help="GA fallback: +/- days window around target date (default 0 = exact)",
    )
    ap.add_argument(
        "--print-stac",
        action="store_true",
        help="GA fallback: print candidate STAC item titles for debugging",
    )
    ...
    if args.bucket:
        try:
            ...
            if (not used_s3) and base_prefix is not None:
                print(
                    "[INFO] No matches with base-prefix; retrying without base-prefix."
                )
                prefixes_no_base = _candidate_prefixes(args.tile, None)
                used_s3 = _download_from_s3(
                    s3, prefixes_no_base, args.tile, args.date, dest_root
                )
        except Exception as e:
            print(
                f"[WARN] S3 path failed to initialize or access (will try GA fallback): {e}"
            )
            used_s3 = False
    ...
    if args.bbox:
        ...
        try:
            ...
            raise SystemExit(
                "Invalid --bbox; expected four comma-separated numbers: minx,miny,maxx,maxy"
            )
    ...
    print(
        f"[GA] Inline fallback querying collections for {path}_{row} on {target_date_iso}{msg_bbox}"
    )
    ...
    def _stac_query(collection: str) -> Optional[dict]:
        ...
        if bbox_clean:
            url = (
                f"{base}/search?collections={collection}"
                f"&datetime={dt_param}"
    ...
    if not feature:
        print(
            "[GA] No scene found within search window; consider increasing --search-days or --cloud-threshold."
        )
        return 1
            ...
            try:
                ...
                if not _download_ok:
                    ...
                    if href.startswith("http://") or href.startswith("https://"):
                        with urllib.request.urlopen(href) as r, open(
                            out_path, "wb"
                        ) as w:
                            w.write(r.read())
                        _download_ok = True
                if not _download_ok:
                    raise RuntimeError(
                        f"Unsupported or inaccessible asset href: {href}"
                    )
                time.sleep(0.25)
            except Exception as e:
    ...
    try:
        ...
        out_name = (
            f"{title.split('_3-2-1_')[0]}_{path}{row}_{td_norm}_srb{sr_count}.tif"
        )
        ...
        if composite_path.exists():
            ...
            print(
                f"[GA] Creating composite {composite_path.name} with {sr_count} bands"
            )
            dst = driver.Create(
                str(composite_path),
                first.RasterXSize,
                first.RasterYSize,
                sr_count,
                first.GetRasterBand(1).DataType,
            ...
            for b_index, band_path in enumerate(local_band_paths, start=1):
                ...
                dst.GetRasterBand(b_index).SetNoDataValue(
