...
maskTypes_other = [masks.MT_WATERNDX, masks.MT_TOPOINCIDENCE, masks.MT_TOPOCASTSHADOW, masks.MT_SNOW]
...
def getCmdargs():
    ...
    """
    p = argparse.ArgumentParser(description=("Run the top-of-atmosphere-based clearing detection  model. " +
                                             "Incorporates prior timeseries of FPC, and simple topo-correction."))
    p.add_argument("--scene", help="Name of scene to process")
    p.add_argument("--era",
                   help=("Change era to process. If given as a year pair (e.g. e1213, meaning 2012-2013) " +
    ...
    p.add_argument("--startdateimage",
                   help="File name for start date image (db8 stage). If using this, do not use --era or --scene. ")
    p.add_argument("--enddateimage",
                   help="File name for end date image (db8 stage). If using this, do not use --era or --scene. ")
    p.add_argument("--reportinputs",
                   help=("Give the name of a file in which to list the input " +
                         "files which would be required for this run. Does not actually execute anything, and exits " +
                         "after creating this list file. "))
    p.add_argument("--omitcloudmasks", default=False, action="store_true",
                   help="Omit the cloud masks from the start/end date image masking")
    p.add_argument("--writeindexes", default=False, action="store_true",
                   help=("Write secondary output file of component indexes (default does not write this). " +
                         "Layers are (combinedIndex, spectralIndex, fpcDiff, sTest, tTest). File name is " +
                         "same as output, with _zindex"))
    p.add_argument("--timeseriesdates",
                   help=("Comma-separated list of specific dates to use for timeseries stats. No spaces. " +
    ...
    p.add_argument("--clipqldstate", default=False, action="store_true", help="Clip to QLD state boundary")
...
def mainRoutine():
    ...
    (changeClass, changeInterp) = runChangeModel(refPair, normedFpcPair, fpcPair[0], timeseriesStatsImg, footprintFile,
                                                 cmdargs, stages, DBcursor)
...
def getImagePairs(cmdargs, DBcursor, stages):
    ...
    if cmdargs.startdateimage is not None and cmdargs.enddateimage is not None:
        ...
        cmdargs.era = "d{}{}".format(qvf.when(cmdargs.startdateimage), qvf.when(cmdargs.enddateimage))
...
def getFpcTimeseries(fpcPair, DBcursor, cmdargs):
    ...
    dateList = [(sat, instr, date) for (sat, instr, date) in dateList if date <= startDate]
    ...
    for (satellite, instrument, date) in dateList:
        fpcImg = qvf.assemble([satellite + instrument + 're', scene, date, stage + 'mz']) + '.img'
...
def findSimilarSeasonDates(scene, targetdate, DBcursor, cloudthresh):
    ...
    months = [(m - 1) % 12 + 1 for m in range(targetMonth - seasonTol, targetMonth + seasonTol + 1)]
    ...
    sql = """
        ...
        where 
            ...
            pcntcloud < %s
        order by date
    """ % (monthSet, yearSet, scene, cloudthresh)
    ...
    for year in range(startYear, endYear + 1):
        datesThisYear = [(sat, instr, date, cld) for (sat, instr, date, cld) in
                         results if date[:4] == '%4.4d' % year]
        if len(datesThisYear) > 0:
            minCloudVal = min(datesThisYear, key=lambda x: x[-1])[-1]
            minCloudDates = sorted(
                [(date, sat, instr) for (sat, instr, date, cld) in datesThisYear if cld == minCloudVal])
...
def runChangeModel(refPair, normedFpcPair, fpcStart, statsImg, footprintFile, cmdargs, stages, DBcursor):
    """
    ...
    infiles.masklist = [masks.getAvailableMaskname(refPair[i], mt) for i in range(2) for mt in maskTypesForChange]
    ...
    if cmdargs.writeindexes:
        outfiles.allindexes = qvf.changeoptionfield(outfiles.changeclass, 'z', 'indexes')
    ...
    otherargs.maskRecodeVals = [108, 107, 107, 109, 103, 105, 108, 107, 107, 109, 104, 106]
...
def doChangeModel(info, inputs, outputs, otherargs):
    ...
    if otherargs.writeindexfile:
        ...
        fullMask = (outputs.changeclass[0] > 100) | (outputs.changeclass[0] == NULL_CLEARING)
...
def stretchInterp(info, inputs, outputs):
    ...
    spectralStretched = stretch(spectralIndex, spectralMean, spectralStddev, 2, 1, 255, 0)
    sTestStretched = stretch(sTest, sTestMean, sTestStddev, 10, 1, 255, 0)
    combinedStretched = stretch(combinedIndex, combinedMean, combinedStddev, 10, 1, 255, 0)
    ...
    outStack = numpy.array([spectralStretched, sTestStretched, combinedStretched, clearingProb],
                           dtype=numpy.uint8)
...
def addClrTbl(changeClass, era):
    ...
    clrTblFile = os.path.expandvars("$QV_IMGTEMPLATEDIR/clearing_colours_%s.txt" % eraStr)
    if not os.path.exists(clrTblFile):
        clrTblFile = os.path.expandvars("$QV_IMGTEMPLATEDIR/clearing_colours_fallback.txt")

    ratFile = os.path.expandvars("$QV_IMGTEMPLATEDIR/clearing_class_names_%s.dat" % eraStr)
...
def getStateBoundary(imgfile, DBcursor):
    ...
    if ((row == 80) or (path == 100) or
            (scene in ['p097r079', 'p097r078', 'p098r078', 'p099r078', 'p099r077', 'p099r076',
                       'p101r071', 'p101r072', 'p099r066'])):
...
class InputSpecificationError(Exception): pass
