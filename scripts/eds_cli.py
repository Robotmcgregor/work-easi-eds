...
def cmd_list_s3(args: argparse.Namespace) -> int:
    ...
    base_prefix = args.base_prefix if args.base_prefix is not None else (cfg.base_prefix or "")
    if not bucket:
    ...
    count = 0
    try:
        for key in s3.list_prefix(prefix, recursive=not args.shallow, max_keys=args.limit or None):
            print(key)
            count += 1
    except Exception as e:
        print("S3 list failed:", e)
        print("Hint: Ensure the credentials/profile have s3:ListBucket permission for the bucket, and that the bucket policy allows listing for your principal.\n"
        ...
        return 3
    if count == 0 and args.prefix is None and args.tile_id.isdigit() and len(args.tile_id) == 6:
        ...
        try:
            for key in s3.list_prefix(alt_prefix, recursive=not args.shallow, max_keys=args.limit or None):
                print(key)
                count += 1
        except Exception as e:
...
def cmd_fetch_inputs(args: argparse.Namespace) -> int:
    ...
    paths = da.ensure_local_inputs(args.tile_id, start, end, prefix=args.prefix, download=True, max_files=args.limit)
...
def cmd_download_s3(args: argparse.Namespace) -> int:
    ...
    if args.prefix:
        ...
        with_us = f"{args.tile_id[:3]}_{args.tile_id[3:]}/" if len(args.tile_id) == 6 and args.tile_id.isdigit() else f"{args.tile_id}/"
        prefixes = [no_us, with_us]
        base_prefix_use = base_prefix

    # If year provided, append year/ to narrow listing
    if args.year:
        prefixes = [ ( (f"{base_prefix_use}/{p}{args.year}/") if base_prefix_use else f"{p}{args.year}/" ) for p in prefixes ]
    else:
        prefixes = [ ( (f"{base_prefix_use}/{p}") if base_prefix_use else p ) for p in prefixes ]
...
def cmd_upload_s3(args: argparse.Namespace) -> int:
    ...
    tile_us = f"{args.tile_id[:3]}_{args.tile_id[3:]}" if len(args.tile_id) == 6 and args.tile_id.isdigit() else args.tile_id
    sensors = set([s.lower() for s in args.sensor]) if args.sensor else None
    types = set([t.lower() for t in args.type]) if args.type else None
...
def cmd_usgs_login(args: argparse.Namespace) -> int:
    ...
    app_token = (getattr(args, 'token', None) or getattr(cfg, 'token', '') or '').strip()
    ...
    if not app_token and not password:
        print("Provide either USGS_PASSWORD or USGS_TOKEN via .env, or use --password/--token flags.")
        return 2
    try:
        client = USGSM2MClient(username, password=None if app_token else password, endpoint=endpoint, app_token=app_token or None)
        _ = client.login()
...
def cmd_usgs_search(args: argparse.Namespace) -> int:
    ...
    try:
        client = USGSM2MClient(cfg.username, cfg.password, endpoint=cfg.endpoint, app_token=getattr(cfg, 'token', '') or None)
        client.login()
        scenes = client.search_scenes_wrs2(dataset, p, r, start, end, node=node, max_results=args.limit)
...
def cmd_usgs_download(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if not scenes:
            ...
            client.logout()
            return 0
        entity_ids = [s.get('entityId') or s.get('entity_id') for s in scenes if s.get('entityId') or s.get('entity_id')]
...
def cmd_usgs_search_bbox(args: argparse.Namespace) -> int:
    ...
    try:
        client = USGSM2MClient(cfg.username, cfg.password, endpoint=cfg.endpoint, app_token=getattr(cfg, 'token', '') or None)
        client.login()
        scenes = client.search_scenes_bbox(dataset, args.min_lon, args.min_lat, args.max_lon, args.max_lat, start, end, node=node, max_results=args.limit)
        print(f"Found {len(scenes)} scene(s)")
        for s in scenes:
            print(s.get('entityId') or s.get('entity_id'), s.get('displayId') or s.get('display_id'))
        client.logout()
...
def cmd_usgs_search_bbox_tile(args: argparse.Namespace) -> int:
    ...
    if not bbox:
        print(f"No bbox found in DB for tile {args.tile_id}. Run wrs2 import first (see README-create-bbox.md).")
        return 2
    min_lon, min_lat, max_lon, max_lat = bbox
    try:
        client = USGSM2MClient(cfg.username, cfg.password, endpoint=cfg.endpoint, app_token=getattr(cfg, 'token', '') or None)
        client.login()
        scenes = client.search_scenes_bbox(dataset, min_lon, min_lat, max_lon, max_lat, start, end, node=node, max_results=args.limit)
        print(f"Found {len(scenes)} scene(s)")
        for s in scenes:
            print(s.get('entityId') or s.get('entity_id'), s.get('displayId') or s.get('display_id'))
        client.logout()
...
def cmd_usgs_search_recent(args: argparse.Namespace) -> int:
    ...
    try:
        client = USGSM2MClient(cfg.username, cfg.password, endpoint=cfg.endpoint, app_token=getattr(cfg, 'token', '') or None)
        client.login()
        scenes = client.search_recent(dataset, days=args.days, node=node, max_results=args.limit)
...
def cmd_run_tile(args: argparse.Namespace) -> int:
    ...
    if args.start and args.end:
        ...
        cfg = EDSPipelineManager.create_processing_config(days_back=args.days_back, confidence_threshold=float(args.confidence))

    res = EDSPipelineManager.run_tile_processing(args.tile_id, cfg)
    print(f"Success: {res.success} tile: {res.tile_id} alerts: {res.alerts_detected} time(s): {res.processing_time:.2f}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_ls.add_argument("--prefix", help="Optional explicit prefix (overrides base-prefix + tile)")
    ...
    p_ls.add_argument("--endpoint", help="S3 endpoint URL override (for S3-compatible stores)")
    p_ls.add_argument("--role-arn", help="Assume role ARN override")
    p_ls.add_argument("--base-prefix", help="Base prefix override (defaults to config.s3.base_prefix)")
    p_ls.add_argument("--shallow", action="store_true", help="Do not recurse into sub-prefixes")
    p_ls.set_defaults(func=cmd_list_s3)

    p_fetch = sub.add_parser("fetch-inputs", help="Download inputs for a tile + date range to local cache")
    ...
    p_fetch.add_argument("--prefix", help="Optional explicit prefix (overrides base-prefix + tile)")
    ...
    p_fetch.add_argument("--endpoint", help="S3 endpoint URL override (for S3-compatible stores)")
    p_fetch.add_argument("--role-arn", help="Assume role ARN override")
    p_fetch.add_argument("--base-prefix", help="Base prefix override (defaults to config.s3.base_prefix)")
    p_fetch.set_defaults(func=cmd_fetch_inputs)

    # New: download specific S3 files with filters and custom destination
    p_dl = sub.add_parser("download-s3", help="Download S3 files for a tile with filters (year, sensor, type) and preserve S3 folder structure under --dest")
    ...
    p_dl.add_argument("--endpoint", help="S3 endpoint URL override (for S3-compatible stores)")
    p_dl.add_argument("--role-arn", help="Assume role ARN override")
    p_dl.add_argument("--base-prefix", help="Base prefix override (defaults to config.s3.base_prefix)")
    p_dl.add_argument("--prefix", help="Explicit prefix (e.g., 089_078/ or 089_078/2023/)")
    p_dl.add_argument("--year", type=int, help="Filter by year, e.g., 2023")
    p_dl.add_argument("--type", action="append", help="Filter by product type (repeatable), e.g., --type fc3ms --type fmask")
    p_dl.add_argument("--sensor", action="append", help="Filter by sensor family (repeatable): ls8c, ls9c, ls_fc")
    p_dl.add_argument("--limit", type=int, default=1000, help="Maximum files to download")
    p_dl.add_argument("--dest", default="data/cache", help="Destination root directory (preserves key structure)")
    p_dl.add_argument("--overwrite", action="store_true", help="Overwrite local files if they already exist")
    p_dl.set_defaults(func=cmd_download_s3)

    # Upload local files to S3, preserving structure under --source
    p_ul = sub.add_parser("upload-s3", help="Upload local files to S3 preserving folder structure under --source; filter by year, sensor, type")
    p_ul.add_argument("tile_id", help="Tile ID, e.g., 089078 (underscore variant will be preserved from local path)")
    p_ul.add_argument("--source", required=True, help="Local source root directory (e.g., D:\\data\\lsat)")
    ...
    p_ul.add_argument("--endpoint", help="S3 endpoint URL override (for S3-compatible stores)")
    p_ul.add_argument("--role-arn", help="Assume role ARN override")
    p_ul.add_argument("--base-prefix", help="Base prefix under bucket (defaults to config.s3.base_prefix)")
    p_ul.add_argument("--year", type=int, help="Filter by year, e.g., 2023")
    p_ul.add_argument("--type", action="append", help="Filter by product type (repeatable), e.g., --type fc3ms --type fmask")
    p_ul.add_argument("--sensor", action="append", help="Filter by sensor family (repeatable): ls8c, ls9c, ls_fc")
    p_ul.add_argument("--limit", type=int, default=1000, help="Max files to upload")
    p_ul.add_argument("--overwrite", action="store_true", help="Overwrite objects in S3 if they already exist")
    p_ul.add_argument("--dry-run", action="store_true", help="List files that would be uploaded without uploading")
    p_ul.set_defaults(func=cmd_upload_s3)

    # USGS M2M helpers
    p_m2m_login = sub.add_parser("usgs-login", help="Test USGS M2M login using .env or flags")
    ...
    p_m2m_login.add_argument("--token", help="USGS application token (overrides .env USGS_TOKEN)")
    p_m2m_login.set_defaults(func=cmd_usgs_login)

    p_m2m_search = sub.add_parser("usgs-search", help="Search USGS M2M by WRS-2 tile and date range")
    ...
    p_m2m_search.add_argument("--dataset", help="Dataset name (e.g., LANDSAT_8_C2_L1, LANDSAT_9_C2_L1)")
    ...
    p_m2m_dl = sub.add_parser("usgs-download", help="Download from USGS M2M for a tile and date range (TOA: *_C2_L1)")
    p_m2m_dl.add_argument("tile_id")
    p_m2m_dl.add_argument("--start", required=True)
    p_m2m_dl.add_argument("--end", required=True)
    p_m2m_dl.add_argument("--dataset", help="Dataset name (default from config; e.g., LANDSAT_8_C2_L1)")
    ...
    p_m2m_bbox = sub.add_parser("usgs-search-bbox", help="Search USGS M2M by bounding box and date range")
    ...
    p_m2m_recent = sub.add_parser("usgs-search-recent", help="List recent scenes globally (no spatial filter)")
    ...
    p_m2m_bbox_tile = sub.add_parser("usgs-search-bbox-tile", help="Search USGS M2M by tile_id using bbox from DB and date range")
    ...
    p_m2m_bbox_tile.add_argument("--dataset", help="Dataset name or id (e.g., LANDSAT_8_C2_L1)")
