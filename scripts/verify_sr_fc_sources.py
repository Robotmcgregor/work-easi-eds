...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Verify only Landsat 8/9 SR composites present for a tile"
    )
    ap.add_argument(
    ...
    print(
        f"[SUMMARY] SR counts -> LS8: {len(ls8)}, LS9: {len(ls9)}, LS7: {len(ls7)}, LS5: {len(ls5)}"
    )
    if ls5 or ls7:
        print(
            "[WARNING] Found Landsat 5/7 composites (should be excluded). List first 5:"
        )
        for p in (ls5 + ls7)[:5]:
            print("  ", p)
    else:
    ...
    if pre2013:
        print(
            "[NOTE] FC files with dates before 2013 detected; FC product may internally derive from earlier sensors."
        )
