...
def download_file(url, output_path, chunk_size=1024 * 1024):
    ...
    try:
        if url.startswith("s3://"):
            ...
            with urllib.request.urlopen(request, timeout=60) as resp, dest.open(
                "wb"
            ) as dst:
                while True:
...
def find_closest_sr_scene(target_date, path, row, bbox, search_days=30):
    ...
    time_range = (
        f"{start_search.strftime('%Y-%m-%d')}/{end_search.strftime('%Y-%m-%d')}"
    )
    print(
        f"Searching for SR scenes from {start_search.strftime('%Y-%m-%d')} to {end_search.strftime('%Y-%m-%d')}"
    )

    # Priority list for SR collections (restricted to Landsat 8/9 only)
    collections = ["ga_ls9c_ard_3", "ga_ls8c_ard_3"]
                ...
                for feature in data_sr["features"]:
                    ...
                    if pathrow_match:
                        ...
                        if scene_pathrow == target_pathrow:
                            ...
                            scene_date_str = properties.get("datetime", "").split("T")[
                                0
                            ]  # YYYY-MM-DD
                            if scene_date_str:
                                ...
                                if platform in ALLOWED_SENSORS:
                                    ...
                                    print(
                                        f"Found scene: {title} (±{date_diff} days) [{platform}]"
                                    )
    ...
    print(
        f"Closest scene: {closest_scene['feature']['properties']['title']} "
...
def process_sr_date(target_date, path, row, dest_dir, bbox, search_days=30):
    ...
    actual_output_base = (
        Path(dest_dir) / pathrow_folder / actual_year / actual_yearmonth
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Ensure SR start and end dates exist for EDS processing"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=30,
        help="Search window in days (±) for closest scenes (default: 30)",
    )
    ...
    start_success, start_filename = process_sr_date(
        args.start_date, path, row, args.dest, bbox, args.search_days
    )
    end_success, end_filename = process_sr_date(
        args.end_date, path, row, args.dest, bbox, args.search_days
    )
