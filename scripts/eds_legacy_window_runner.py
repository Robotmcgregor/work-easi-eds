...
def pick_start_end(
    dc4_files: List[str], year: int, start_mmdd: str, end_mmdd: str
) -> Tuple[str, str]:
    ...
    if not candidates:
        raise SystemExit(
            f"No dc4 images found in {year} within window {start_mmdd}-{end_mmdd}"
        )
...
def build_db8_for_dates(
    scene: str, sr_root: Path, start_date: str, end_date: str
) -> Tuple[str, str]:
    ...
    if not sd_dir.exists():
        raise SystemExit(
            f"SR directory missing for start date: {sd_dir}. Run with --ensure-sr to fetch it."
        )
    if not ed_dir.exists():
        raise SystemExit(
            f"SR directory missing for end date: {ed_dir}. Run with --ensure-sr to fetch it."
        )
    ...
    cmd = [
        _sys.executable,
        str(THIS_DIR / "slats_compat_builder.py"),
        "--tile",
        scene,
        "--sr-date",
        start_date,
...
def maybe_ensure_sr(scene: str, date: str, sr_root: Path, ensure: bool) -> None:
    ...
    cmd = [
        _sys.executable,
        str(THIS_DIR / "ensure_sr_from_s3_or_ga.py"),
        "--tile",
        tile,
        "--date",
        date,
...
def run_legacy(
    start_db8: str, end_db8: str, dates_list: List[str], omit_cloud_masks: bool = False
) -> Tuple[str, str]:
    import subprocess, sys as _sys

    cmd = [
        _sys.executable,
        str(THIS_DIR / "edsadhoc_timeserieschange_d.py"),
        "--startdateimage",
        start_db8,
        "--enddateimage",
        end_db8,
    ...
    m = re.search(
        r"lztmre_(p\d{3}r\d{3})_(\d{8})_db8mz\.img",
        os.path.basename(start_db8),
        re.IGNORECASE,
    )
    scene = m.group(1)
    start = m.group(2)
    end = parse_date_from_name(end_db8)
    era = f"d{start}{end}"
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(
        description="Seasonal-window runner for legacy change detection"
    )
    ...
    ap.add_argument(
        "--ensure-sr",
        action="store_true",
        help="Fetch SR+FMASK for start/end dates before building db8",
    )
    ...
    start_date, end_date = pick_start_end(
        dc4_files, args.year, args.window_start, args.window_end
    )
    print(f"Window start/end dates: {start_date} -> {end_date}")

    # Build seasonal baseline dates list
    base_dates = baseline_dates(
        dc4_files,
        args.year,
        start_date,
        args.window_start,
    ...
    if len(base_dates) < 2:
        print(
            "[WARN] Baseline has <2 dates; legacy will still run but trend robustness is low."
        )
    ...
    # Run legacy
    change, interp = run_legacy(
        s_db8, e_db8, base_dates, omit_cloud_masks=args.omit_cloud_masks
    )
