...
def polygonize_to_layer(src_band: gdal.Band, srs_wkt: str, dst_ds: ogr.DataSource, layer_name: str) -> ogr.Layer:
    # Create layer
...
def export_per_class_shapefiles(src_lyr: ogr.Layer, out_dir: Path, classes: List[int]) -> None:
    ...
    for c in classes:
        ...
        dst_lyr = ds.CreateLayer('polygons', srs=src_lyr.GetSpatialRef(), geom_type=ogr.wkbPolygon)
        fld = ogr.FieldDefn('class', ogr.OFTInteger)
...
def main(argv=None) -> int:
    ...
    ap.add_argument('--classes', nargs='*', type=int, help='Class values to keep (default: all)')
    ...
    ap.add_argument('--per-class-shapefiles', action='store_true', help='Write one Shapefile per class (requires --out-dir)')
    ...
    if not args.per_class_shapefiles and not args.out:
        raise SystemExit('Provide --out for single GeoPackage output, or use --per-class-shapefiles with --out-dir')
    ...
    print(f"[DEBUG] Full unique class list ({len(full_uniqs)}): {full_uniqs.tolist()[:50]}{'...' if len(full_uniqs)>50 else ''}")
    if args.classes:
    ...
    if args.per_class_shapefiles:
        ...
        print(f"[INFO] Reading full raster to numpy array for per-class polygonize ({xsize}x{ysize})")
        ...
        for c in classes:
            ...
            gdal.Polygonize(mem_ds.GetRasterBand(1), mem_msk.GetRasterBand(1), out_lyr, 0, [], callback=None)
        ...
        print(f"[DEBUG] Polygonize produced {feat_count_initial} features before filtering")
        dst_ds = None
