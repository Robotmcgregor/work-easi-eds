...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Seed compat metadb with landsat_list for given dates")
    ...
    ap.add_argument("--write-timeseries-csv", help="Optional CSV to write the final ordered timeseries dates")
    ap.add_argument("--allow-after-start", action="store_true", help="Allow timeseries dates after start date (non-standard but enables sparse runs)")
    ...
    if args.dates:
        ...
        if not (args.root and args.start_yyyymm and args.end_yyyymm):
            raise SystemExit("Provide --dates OR (--root and --start-yyyymm and --end-yyyymm)")
        root = Path(args.root)
        if not root.exists():
    ...
    if not have_pre_start and not args.allow_after_start:
        ...
        print(f"[INFO] Added synthetic pre-start date {synth} for timeseries initialization")
    con.commit()
