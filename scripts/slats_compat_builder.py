...
def _stack_sr(date_tag: str, scene: str, band_sources: Dict[str, str], out_dir: Path) -> str:
    ...
    if len(files) < 4:
        raise RuntimeError("Need at least 4 SR bands (B2,B3,B5,B6) for index computation or provide a *_srb6/_srb7 composite")
...
def _convert_fc_to_fpc(arr: np.ndarray, nodata: Optional[float], k: float, n: float) -> np.ndarray:
...
def _write_fc(date_tag: str, scene: str, fc_file: str, *, convert_to_fpc: bool=False, k: float=0.000435, n: float=1.909, fc_nodata: Optional[float]=None) -> str:
...
def main(argv=None) -> int:
    ap = argparse.ArgumentParser(description="Build SLATS-compatible db8/dc4 files from GA SR/FC")
    ap.add_argument("--tile", required=True, help="Scene code like p090r084")
    ap.add_argument("--sr-date", action="append", help="SR date tag (YYYYMMDD) to build db8 for; repeat twice for start/end")
    ap.add_argument("--sr-dir", action="append", help="Directory per SR date containing *_B2*.tif, *_B3*.tif, ...")
    ap.add_argument("--fc", action="append", help="Paths or globs to FC green files to include in timeseries (supports ** with recursive)")
    ap.add_argument("--fc-list", help="Optional text file with one FC filepath per line to include")
    ap.add_argument("--fc-only-clr", action="store_true", help="Use only masked FC files (*_fc3ms_clr.tif); ignore unmasked *_fc3ms.tif")
    ap.add_argument("--fc-prefer-clr", action="store_true", help="Prefer clr variant when both *_fc3ms.tif and *_fc3ms_clr.tif for same date exist (default if neither clr-only nor prefer flag supplied is to deduplicate picking clr).")
    
    # FC-> FPC conversion options
    ap.add_argument("--fc-convert-to-fpc", action="store_true", help="Convert FC green to FPC using FPC=100*(1-exp(-k*FC^n)) before writing dc4")
    ...
    ap.add_argument("--fc-nodata", type=float, default=None, help="Override nodata value for FC input; defaults to band nodata if present")
    ...
    for d, ddir in zip(args.sr_date, args.sr_dir):
        ...
        if os.path.isfile(ddir) and re.search(r"_srb[67]\.tif$", os.path.basename(ddir), re.IGNORECASE):
    ...
    if fc_files:
        ...
        if args.fc_only_clr:
            ...
            for d, flist in grouped.items():
                ...
                base = [f for f in flist if f.lower().endswith('_fc3ms.tif') and not f.lower().endswith('_fc3ms_clr.tif')]
            ...
            if len(selected) != len(fc_files):
                print(f"[INFO] Deduplicated FC inputs {len(fc_files)} -> {len(selected)} (preferring clr where available)")
            fc_files = selected
        for fc_file in fc_files:
            try:
                dtag = _extract_date_from_path(fc_file)
                out = _write_fc(
                    dtag, args.tile, fc_file,
                    convert_to_fpc=args.fc_convert_to_fpc,
                    k=args.fc_k, n=args.fc_n, fc_nodata=args.fc_nodata,
                )
                print(out)
    ...
    if built:
        ...
        scene = args.tile
        prod = list((Path.cwd() / Path(stdProjFilename(f"lztmre_{scene}_00000000_dc4mz.img")).parent).glob("*dc4mz.img"))
        if prod:
