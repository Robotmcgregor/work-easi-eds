...
def inspect(cls_path: str, int_path: str):
    ...
    for i in range(1, ids.RasterCount + 1):
        ...
        arr = b.ReadAsArray()
        print(
            f" band {i} -> dtype {arr.dtype}, min {np.nanmin(arr)}, max {np.nanmax(arr)}"
        )
...
def main():
    ...
    ap.add_argument(
        "--scene", help="Scene code to autodetect latest files, e.g., p089r080"
    )
    ap.add_argument(
        "--pattern", help="Glob pattern to locate files if --scene not given"
    )
    args = ap.parse_args()
    ...
    if not args.cls or not args.dlj:
        if args.scene:
            ...
            if not cand:
                cand = sorted(
                    glob.glob(str(base / f"lztmre_{args.scene}_e*_dllmz.img"))
                )
            if not cand:
