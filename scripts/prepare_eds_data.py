...
def main():
    parser = argparse.ArgumentParser(description="Prepare all data required for EDS processing")
    ...
    parser.add_argument("--dest", required=True, help="Destination directory (e.g., D:\\data\\lsat)")
    parser.add_argument("--start-year", type=int, default=2014, help="Start year for seasonal FC data (default: 2014)")
    parser.add_argument("--end-year", type=int, default=2024, help="End year for seasonal FC data (default: 2024)")
    ...
    parser.add_argument("--skip-sr", action="store_true", help="Skip SR date checking (assume they exist)")
    parser.add_argument("--skip-fc", action="store_true", help="Skip FC seasonal data download")
    ...
    print(f"    │   ├── ga_ls*c_ard_{args.tile.replace('_', '')}_{args.start_date}_srb*.tif")
    print(f"    │   ├── ga_ls*c_ard_{args.tile.replace('_', '')}_{args.end_date}_srb*.tif")
