...
def main():
    ...
    with db.get_session() as s:
        ...
        nvms_clearing = safe(
            "SELECT COUNT(*) FROM nvms_detections WHERE (properties->>'IsClearing')='y'"
        )
        ...
        recent = safe(
            "SELECT COUNT(*) FROM nvms_detections WHERE (properties->>'IsClearing')='y' AND imported_at > now() - interval '30 days'"
        )
