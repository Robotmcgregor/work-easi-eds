...
BASE = (
    os.environ.get("USGS_M2M_ENDPOINT")
    or "https://m2m.cr.usgs.gov/api/api/json/stable/"
).rstrip("/") + "/"
...
def _login() -> str:
    if TOKEN:
        ...
        return r.json()["data"]
    r = requests.post(
        BASE + "login", json={"username": USERNAME, "password": PASSWORD}, timeout=30
    )
    r.raise_for_status()
    return r.json()["data"]
...
def cmd_datasets(args: argparse.Namespace) -> int:
    ...
    for ep in ("datasets", "dataset-search"):
        ...
        for pl in payloads:
            ...
            resp = requests.post(
                url, headers={"X-Auth-Token": api_key}, json=pl, timeout=60
            )
    ...
    for t in tried:
        print(t)
    _ = requests.post(
        BASE + "logout", headers={"X-Auth-Token": api_key}, json={}, timeout=30
    )
    return 1
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_ds = sub.add_parser(
        "datasets", help="List datasets; optionally filter by name fragment"
    )
