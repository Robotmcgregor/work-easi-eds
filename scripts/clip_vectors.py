...
def _ensure_same_srs(geom: ogr.Geometry, src_srs: osr.SpatialReference, dst_srs: osr.SpatialReference) -> ogr.Geometry:
...
def _area_fields(geom: ogr.Geometry, srs: osr.SpatialReference) -> Tuple[float, float]:
    if srs is not None and srs.IsGeographic() == 1:
        ...
        tgt = osr.SpatialReference(); tgt.ImportFromEPSG(3577)
...
def main(argv=None) -> int:
    ...
    ap.add_argument('--input-dir', required=True, help='Directory of .shp files to clip')
