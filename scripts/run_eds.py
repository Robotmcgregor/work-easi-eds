...
def start_dashboard(config_file=None, host=None, port=None, debug=None):
    ...
    try:
        ...
        logger.info(f"Dashboard starting on http://{dashboard_config.host}:{dashboard_config.port}")
        app.run_server(
...
def run_processing(config_file=None, tile_id=None, all_tiles=False, pending_only=False, days_back=7):
    """Run EDS processing."""
    try:
        ...
        processing_config = EDSPipelineManager.create_processing_config(days_back=days_back)
        ...
        if tile_id:
            ...
            if result.success:
                logger.info(f"Processing successful: {result.alerts_detected} alerts detected")
            else:
            ...
            logger.info(f"Pending processing complete: {successful}/{len(results)} successful, "
                       f"{total_alerts} total alerts detected")
...
def main():
    ...
    dashboard_parser.add_argument('--port', type=int, help='Dashboard port (default from config)')
    dashboard_parser.add_argument('--debug', action='store_true', help='Enable debug mode')
    
    # Scheduler command  
    scheduler_parser = subparsers.add_parser('scheduler', help='Start the processing scheduler')
    ...
    process_group.add_argument('--all-tiles', action='store_true', help='Process all tiles')
    process_group.add_argument('--pending-only', action='store_true', help='Process only pending/failed tiles')
    process_parser.add_argument('--days-back', type=int, default=7, help='Days back for time range (default: 7)')
