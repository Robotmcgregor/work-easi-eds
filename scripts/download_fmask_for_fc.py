...
def search_stac_for_fmask(bbox, fc_date_str, path, row, search_window_days=7):
    ...
    sr_collections = ["ga_ls9c_ard_3", "ga_ls8c_ard_3", "ga_ls7e_ard_3", "ga_ls5t_ard_3"]
                ...
                for item in data["features"]:
                    ...
                    if target_pathrow in fmask_url or f"{path}/{row}" in fmask_url:
                        ...
                        if date_diff < best_date_diff:
                            ...
                            print(f"    Better match: {item_date_str} (diff: {date_diff} days)")
...
def main():
    parser = argparse.ArgumentParser(description="Download FMASK files for existing FC data")
    parser.add_argument("--tile", required=True, help="Tile in format 089_078")
    parser.add_argument("--root", default="D:\\data\\lsat", help="Root directory containing tile data")
    ...
    for fc_file in fc_files:
        ...
        stac_item, fmask_asset = search_stac_for_fmask(bbox, date_formatted, path, row, search_window_days=7)
