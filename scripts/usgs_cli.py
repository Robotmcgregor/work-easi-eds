...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_login = sub.add_parser("usgs-login", help="Test USGS M2M login using .env or flags")
    ...
    p_login.add_argument("--token", help="USGS application token (overrides .env USGS_TOKEN)")
    p_login.add_argument("--endpoint", help="USGS M2M endpoint override")
    p_login.set_defaults(func=cmd_usgs_login)

    p_search = sub.add_parser("usgs-search", help="Search USGS M2M by WRS-2 tile and date range")
    p_search.add_argument("tile_id")
    p_search.add_argument("--start", required=True)
    p_search.add_argument("--end", required=True)
    p_search.add_argument("--dataset", help="Dataset name (e.g., LANDSAT_8_C2_L1, LANDSAT_9_C2_L1)")
    p_search.add_argument("--node", default="EE", help="Node (default EE)")
    p_search.add_argument("--limit", type=int, default=50)
    p_search.set_defaults(func=cmd_usgs_search)

    p_dl = sub.add_parser("usgs-download", help="Download from USGS M2M for a tile and date range (TOA: *_C2_L1)")
    p_dl.add_argument("tile_id")
    p_dl.add_argument("--start", required=True)
    p_dl.add_argument("--end", required=True)
    p_dl.add_argument("--dataset", help="Dataset name (default from config; e.g., LANDSAT_8_C2_L1)")
    ...
    p_bbox = sub.add_parser("usgs-search-bbox", help="Search by bounding box and date range")
    ...
    p_recent = sub.add_parser("usgs-search-recent", help="List recent scenes globally (no spatial filter)")
    ...
    p_bbox_tile = sub.add_parser("usgs-search-bbox-tile", help="Search by tile_id using DB bbox and date range")
