...
from src.utils.tile_management import initialize_australian_tiles, get_australian_tile_grid
...
def reset_tile_status():
    ...
    try:
        ...
        for tile in all_tiles:
            if TileManager.update_tile_status(tile.tile_id, "pending", "Reset by initialization script"):
                reset_count += 1
...
def export_tile_list(output_file):
    ...
    try:
        ...
        with open(output_file, 'w', newline='') as csvfile:
            fieldnames = [
                'tile_id', 'path', 'row', 'center_lat', 'center_lon', 
                'status', 'last_processed', 'is_active', 'priority'
            ]
            ...
            for tile in tiles:
                writer.writerow({
                    ...
                    'status': tile.status,
                    'last_processed': tile.last_processed.isoformat() if tile.last_processed else '',
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Initialize Landsat tiles for the EDS system")
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--stats-only",
        action="store_true", 
        help="Only show tile grid statistics"
    )
    parser.add_argument(
        "--status-only",
        action="store_true",
        help="Only show database tile status"
    )
    parser.add_argument(
        "--reset-status",
        action="store_true",
        help="Reset all tiles to pending status"
    )
    parser.add_argument(
        "--export-csv",
        help="Export tile list to CSV file"
    )
